<?php
/**
 * Webkul Odoomagentoconnect Category MassDisable Controller
 *
 * @category  Webkul
...
/**
 * Webkul Odoomagentoconnect Category MassDisable Controller class
 */
class MassDisable extends \Magento\Backend\App\Action
{
    ...
    public function __construct(
        Context $context,
        Filter $filter,
        \Webkul\Odoomagentoconnect\Model\Category $model
    ) {
        $this->_filter = $filter;
        $this->_model = $model;
    ...
    public function execute()
    {
        try {
            ...
            if ($collection->getSize() > 0) {
                ...
                $collection->save();
                $this->messageManager->addSuccess(__('Category Need Sync disabled successfully.'));
            }
        } catch (\Exception $e) {
