...
namespace MyFatoorah\EmbedPay\Controller\Checkout;
...
//---------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    private function postToCheckout($order) {
        ...
        $sessionId = $this->getRequest()->get('sid') ?: 'undefined';
        ...
        $data     = $this->mfObj->embeddedPayment($curlData, $sessionId, $this->orderId);

        //save the invoice id in myfatoorah_invoice table 
        $mf = $this->objectManager->create('MyFatoorah\EmbedPay\Model\MyfatoorahInvoice');
        $mf->addData([
