...
//---------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    function getInvoiceItems($order, $currencyRate, $isMFShipping, &$amount, $isPayment = false) {
        ...
        //AmstorecreditAmount
        $amstorecreditAmount = $order->getAmstorecreditAmount();
        if ($amstorecreditAmount != '0') {
            $itemPrice         = -round($amstorecreditAmount * $currencyRate, 2);
            $invoiceItemsArr[] = ['ItemName' => 'Amstorecredit Amount', 'Quantity' => '1', 'UnitPrice' => $itemPrice, 'Weight' => '0', 'Width' => '0', 'Height' => '0', 'Depth' => '0'];
