...
namespace MyFatoorah\MyFatoorahPaymentGatewayDummy\Model;

use MyFatoorah\MyFatoorahPaymentGatewayDummy\Api\MFOrderManagementInterface;

//use Magento\Sales\Model\Order;
//use MyFatoorah\MyFatoorahPaymentGatewayDummy\Controller\Checkout\AbstractAction;
...
//        $order = $objectManager->create('Magento\Sales\Model\Order')->loadByIncrementId($incrementId);
        ...
        $returnURL = str_replace('?___SID=U', '', $objectManager->create('Magento\Framework\UrlInterface')->getUrl('dummymyfatoorah/checkout/apiresponse?apicid='. base64_encode($cartId), array('_secure' => true)));

        $currencyCode = $objectManager->create('Magento\Store\Model\StoreManagerInterface')->getStore()->getCurrentCurrency()->getCode();

        $this->token = $objectManager->get('Magento\Framework\App\Config\ScopeConfigInterface')->getValue('payment/myfatoorah_gatewaydummy/api_key');

        $istest = $objectManager->get('Magento\Framework\App\Config\ScopeConfigInterface')->getValue('payment/myfatoorah_gatewaydummy/is_testing');
