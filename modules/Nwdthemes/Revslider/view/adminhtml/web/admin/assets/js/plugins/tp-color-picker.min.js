...
(function($) {
	...
	function buildGradientInput(obj) {
		...
		setEasingRow(ease);
		easing.val(ease).trigger('change.select2RS');
	...
	function init() {
		...
		$('.revbuilder-cpicker-main-btn').on('click.revcpicker', function() {
			...
			if(colorView) {
				...
				if(!gradViewed) {
					...
					gradCore.children('.revbuilder-cpicker-color').not('.blank').eq(0).click();
					gradViewed = true;
			...
			if(!colorView) {
				...
				supressClick = true;
				$('.revbuilder-cpicker-point-bottom').first().click();
				supressClick = false;
		...
		if($.fn.select2RS) {
			...
			presets.select2RS({minimumResultsForSearch: -1});
			easing.select2RS({minimumResultsForSearch: -1});
...
*/
	...
	$.rsColorPickerApi = {
		...
		showGlobal : function(_,colors,candidates) {
			...
			m = '<div id="revbuilder-cpicker-globals"><div id="revbuilder-cpicker-gactive">'+RVS_LANG.globalcoloractive+'</div>';
			...
			cPicker.find('#revbuilder-cpicker').append(m = jQuery(m));
			s = m.find('#gc_skinPicker');
			...
			if (RVS!==undefined && RVS.F!==undefined && RVS.F.initOnOff!==undefined) {
				RVS.F.initOnOff(m);
				RVS.F.turnOnOff(m,false)
			}


			s.select2RS({tags:false,templateResult:formatSkinColor, minimumResultsForSearch:"Infinity"});
	...
	$.fn.rsColorPicker = function(settings) {
		...
		return this.each(function() {
			...
			var wrap = $('<span class="revbuilder-colorpicker"></span>').data('revcolorinput', $this),
				box = $('<span class="revbuilder-colorbox" />'),
				btn = $('<span class="revbuilder-colorbtn" />'),
			...
			btn[0].innerHTML = txt || defTitle || langColor || lang.color;
			$this.attr('data-color', val).data('tpcp', box).addClass('revbuilder-cpicker-component');
