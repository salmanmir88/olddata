...
/************************************************
 * REVOLUTION 6.2.11 EXTENSION - LAYER ANIMATION
 * @version: 6.2.11 (05.06.2020)
...
jQuery.extend(true,_R, {
	...
	requestLayerUpdates : function(id,type,layerid,tPE) {
		...
		if (layerid!==undefined) {
			...
			if (_R[id]._L[i].pVisRequest!==_R[id]._L[i].pVisStatus) {
				...
				} else  _R[id]._L[i].p[0].classList.add("rs-forcehidden");
			...
			if (tPE!==undefined && tPE!=="ignore" && tPE!==0) {
				...
				if  (type==="enterstage" || type==="leavestage" || type==="framestarted") {
					...
					if (!window.isSafari11 && _R[id]._L[i].p[0].style.perspective.length===0 || _R[id]._L[i].p[0].style.perspective=="none") _R[id]._L[i].p[0].style.perspective = tPE+"px";
				} else
				if (type==="frameended") {
					//For FireFox
					if (_R.isFirefox(id)) _R[id]._L[i].p[0].style.transform = _R[id]._L[i].p[0].style.transform.replace('perspective('+tPE+"px"+')','');
					else // For not Safari
					if (!window.isSafari11) _R[id]._L[i].p[0].style.perspective = _R[id]._L[i].p[0].style.perspective.replace(tPE+"px",'');
				}
			}
		} else
		for (i in _R[id]._L) {
		...
		if (type==="enterstage") {
			...
			if (_R[id].middleZones && _R[id].middleZones.length>0 && _R[id].middleZones[_actli]!==undefined ) for (i=0;i<_R[id].middleZones[_actli].length;i++) tpGS.gsap.set(_R[id].middleZones[_actli][i],{y:Math.round(_R[id].height/2 - _R[id].middleZones[_actli][i].offsetHeight/2)+"px"});
			if (_R[id].smiddleZones && _R[id].smiddleZones.length>0) for (i=0;i<_R[id].smiddleZones.length;i++) tpGS.gsap.set(_R[id].smiddleZones[i],{y:Math.round(_R[id].height/2 - _R[id].smiddleZones[i].offsetHeight/2)+"px"});
	...
	initLayer : function(o) {
		...
		for (var li in _R[id].layers[o.skey]) {
			...
			var offsetx = _R[id].sliderType==="carousel" ? 0 : _R[id].width/2 - (_R.iWA(id,o.slideIndex)*_R[id].bw)/2, offsety=0;
			/****************************
			...
			*****************************/
			...
			if (linited===undefined) {
				...
				*************/
				...
				_.esginside = jQuery(L.find('.esg-grid'));
			...
			****************************/
			...
			var gw = _.basealign==="grid" ? _R[id].width : _R[id].sliderType==="carousel" && !_._isstatic ? _R[id].carousel.slide_width :  _R[id].width,
				gh = _R[id].useFullScreenHeight ? _R[id].height :  _.basealign==="grid" ? _R[id].height : _R[id].sliderType==="carousel" && !_._isstatic ? _R[id].height : _R[id].height,//_R[id].carousel.slide_height :  _R[id].height;
				elx = _.x[_R[id].level],
				ely = _.y[_R[id].level];

			//OFFSET CALCULATIONS
			offsety = _.basealign==="slide" ? 0 : Math.max(0,_R[id].sliderLayout=="fullscreen" ? gh/2 - (_R.iHE(id)*(_R[id].keepBPHeight ? 1 :_R[id].bh))/2 : (_R[id].autoHeight || (_R[id].minHeight!=undefined && _R[id].minHeight>0)) ? _R[id].conh/2 - (_R.iHE(id)*_R[id].bh)/2: offsety);
			...
			if (_.basealign!=="slide" && _R[id].sliderType==="carousel" && _._isstatic && _R[id].carousel!==undefined && _R[id].carousel.horizontal_align!==undefined)
				offsetx = Math.max(0, (_R[id].carousel.horizontal_align==="center" ? 0 + (_R[id].width - (_R.iWA(id,'static')*_R[id].bw))/2 : _R[id].carousel.horizontal_align==="right" ?(_R[id].width - (_R[id].gridwidth[_R[id].level]*_R[id].bw)) : offsetx));
			...
			if (o.mode!=="updateposition") {
				...
				if (_.layertype==="video") { // IF IT IS A VIDEO LAYER
					if (_R.manageVideoLayer && !_.videoLayerManaged) _R.manageVideoLayer(L,id,skey);
					...
					ww = ww==="auto" ? ww : (!jQuery.isNumeric(ww) && ww.indexOf("%")>0) ? _._incolumn || _._ingroup ? "100%" : _.basealign==="grid" ? _R.iWA(id,o.slideIndex)*_R[id].bw : gw : _.rsp_bd!=="off" ? (parseFloat(ww)*_R[id].bw)+"px" : parseFloat(ww)+"px";
					hh = hh==="auto" ? hh : (!jQuery.isNumeric(hh) && hh.indexOf("%")>0) ? _.basealign==="grid" ? _R.iHE(id)*_R[id].bw : gh : _.rsp_bd!=="off" ? (parseFloat(hh)*_R[id].bh)+"px" : parseFloat(hh)+"px";
					...
					if (_._incolumn && (ww==="100%" || hh==="auto") && _.ytid!==undefined) {
						...
						if (L[0].className.indexOf('rs-fsv')==-1 && !yvcover) {
							hh = hh==="auto" && _.vd!==undefined  && ww!=="auto" ? ww/_.vd : hh;
						   ...
						   _.vidOBJ = {width:ww /*gw*/, height:(_R[id].autoHeight ? _R[id].conh : hh /*gh*/)};
						}
						if ((_.html5vid == false && !yvcover) || ((_.forcecover!=1 && !L.hasClass('rs-fsv') && !yvcover))) _.mediaOBJ = {width:ww, height:hh, display:"block"};
						if (_._ingroup && _.vidOBJ.width!==null && _.vidOBJ.width!==undefined && !jQuery.isNumeric(_.vidOBJ.width) && _.vidOBJ.width.indexOf("%")>0) _.OBJUPD.lppmOBJ = {minWidth:ww};
					}
			...
			if (o.mode!=="preset" ) {
				...
				var vofs = _.rsp_o==="on" ? parseInt(_.voffset[_R[id].level],0) * _R[id].bw : parseInt(_.voffset[_R[id].level],0),
					...
					crw =  _.basealign==="grid" ? _R.iWA(id,o.slideIndex)*_R[id].bw : gw,
					crh =  _.basealign==="grid" ? _R.iHE(id)*(_R[id].keepBPHeight || _R[id].currentRowsHeight>_R[id].gridheight[_R[id].level] ? 1 :_R[id].bh) : gh;

				if (_R[id].gridEQModule == true || (_._lig!==undefined && _.type!=="row" && _.type!=="column" && _.type!=="group")) {
					crw = _._lig!==undefined ? _._lig.width() : _R[id].width;
					crh = _._lig!==undefined ? _._lig.height() : _R[id].height;
					offsetx=0;
					offsety=0;
				}
				...
				if (_.type==="row") {
					_.OBJUPD.POBJ = {zIndex:_.zindex, width:(_.basealign==="grid" ? crw+"px" : "100%"), top:0,left:offsetx,overwrite:"auto"};
				...
				if (_._ingroup) {
					if (_._groupw!==undefined && !jQuery.isNumeric(_._groupw) && _._groupw.indexOf("%")>0) _.OBJUPD.lppmOBJ.minWidth =  _._groupw;
					if (_._grouph!==undefined && !jQuery.isNumeric(_._grouph) && _._grouph.indexOf("%")>0) _.OBJUPD.lppmOBJ.minHeight =  _._grouph;
				}
 	...
 	animcompleted : function(_nc,id,force) {
		...
		if (_.type!=undefined && _.type!="none")
		 if (_.aplay==true || _.aplay=="true" || _.aplay=="on" ||  _.aplay=="1sttime") {
		 	...
		 	if (_R[id].sliderType!=="carousel" || (_nc.closest('rs-slide').index() == _R[id].carousel.focused) || (_nc.closest('rs-slide').index()==_R[id].activeRSSlide && _R[id].carousel.oldfocused == _R[id].carousel.focused) || force) _R.playVideo(_nc,id);
	...
	*********************************************************/
	...
	updateLayersOnFullStage : function(id) {
		if (_R[id].caches.calcResponsiveLayersList.length>0) {
			if (_R[id].slideHasIframe!==true && _R[id].fullScreenMode!==true) { if (_R[id].sliderType === "carousel") _R[id].carousel.wrap.detach(); else _R[id].canvas.detach();}
			for (var i=0; i<_R[id].caches.calcResponsiveLayersList.length;i++) if (_R[id].caches.calcResponsiveLayersList[i]!==undefined) calcResponsiveLayer(_R[id].caches.calcResponsiveLayersList[i]);
			if (_R[id].slideHasIframe!==true && _R[id].fullScreenMode!==true) { if (_R[id].sliderType === "carousel") _R[id].c[0].appendChild(_R[id].carousel.wrap[0]); else _R[id].c[0].appendChild(_R[id].canvas[0]);}
		}
	},
	...
	*************************************/
	animateTheLayers : function(obj)  {
		...
		if (obj.mode==="preset" && (_R[id].slidePresets===undefined || _R[id].slidePresets[obj.slide]===undefined)) {
			...
			_R[id].c.trigger('revolution.slideprepared',{slide:obj.slide});
		}

		_R[id].lastAnimateLayersCall = {id:id, skey:key, slide:obj.slide,key:key,mode:obj.mode,index:index};
		// MODIFICATED REQUEST ANIMATION BASED START OF ANIMATION
		window.requestAnimationFrame(function() {
			if (_R[id].dimensionReCheck[key]===undefined) {
				_R.updateLayerDimensions(_R[id].lastAnimateLayersCall);
				if (_R[id].doubleDimensionCheck!==true) setTimeout(function() {
					_R.updateLayerDimensions(_R[id].lastAnimateLayersCall);
					_R.updateRowZones(_R[id].lastAnimateLayersCall);
				},150);	else _R.updateRowZones(_R[id].lastAnimateLayersCall);
				_R[id].doubleDimensionCheck = true;
				_R[id].dimensionReCheck[key] = true;
			} else 	_R.updateRowZones(_R[id].lastAnimateLayersCall);

			if (key!==undefined && _R[id].layers[key]) for (var li in _R[id].layers[key]) if (_R[id].layers[key].hasOwnProperty(li)) _R.renderLayerAnimation({layer:jQuery(_R[id].layers[key][li]),id:id, mode:obj.mode,caller:obj.caller});
			if (_R[id].layers.static) for (var li in _R[id].layers.static) if (_R[id].layers.static.hasOwnProperty(li)) _R.renderLayerAnimation({layer:jQuery(_R[id].layers.static[li]),id:id, mode:obj.mode,caller:obj.caller});
			if (_R[id].mtl != undefined) _R[id].mtl.resume();
		});
	},

	updateRowZones : function(obj) {
		if ((_R[obj.id].rowzones!==undefined && _R[obj.id].rowzones.length>0 && obj.index>=0 && _R[obj.id].rowzones[ Math.min(obj.index,_R[obj.id].rowzones.length)].length>0) || (_R[obj.id].srowzones!==undefined && _R[obj.id].srowzones.length>0) || (_R[obj.id].smiddleZones!==undefined && _R[obj.id].smiddleZones.length>0)) {
				_R.setSize(obj.id);
				_R.updateDimensions(obj.id);
	...
	// IN CASE WE WANT TO WORK WITH GOOGLE FONT PRELOADING
	updateLayerDimensionsTimeout : function(id) {
		clearTimeout(_R[id].gFLoadedTimer);
		_R[id].gFLoadedTimer = setTimeout(function() {
			var obj = _R[id].lastAnimateLayersCall;
			if (obj===undefined) return;
			_R.updateLayerDimensions(_R[obj.id].lastAnimateLayersCall);
		},19);
	},
	...
	updateLayerDimensions : function(obj) {
		...
		if (obj.key!==undefined && _R[obj.id].layers[obj.key] && _R.checkLayerDimensions({id:obj.id, skey:obj.key, slideIndex: obj.slide})) {changes = true;_R.initLayer({id:obj.id, skey:obj.key, slideIndex: obj.slide, mode:"updateAndResize"});}
		if (_R.checkLayerDimensions({id:obj.id, skey:'individual', slideIndex: "individual"})) {changes = true;_R.initLayer({id:obj.id, skey:'individual', slideIndex: "individual", mode:"updateAndResize"});}
		if (_R[obj.id].layers["static"] && _R.checkLayerDimensions({id:obj.id, skey:"static",slideIndex: 'static'})) {changes = true;_R.initLayer({	id:obj.id, skey:"static", slideIndex: 'static', mode:"updateAndResize"});}
		if (changes) _R.updateLayersOnFullStage(obj.id);
		//changed updateposition to updateAndResize which will force video and image to reset dimensions within columns and groups with 100% width.
	},
	...
	renderLayerAnimation : function(obj) {
		...
		for (var fiin in _.ford) {
			...
			if (_.splitText!==undefined && _.splitText!==false) {
				for (var i in splitTypes) {
					if (_frameObj[splitTypes[i]]!==undefined && !_.quickRendering) {
						...
						var	$anim = jQuery.extend(true,{},sanim),
							$from = _.fff ? jQuery.extend(true,{},sfrom) : undefined,
			...
			if (_.pxundermask || (mask!==undefined && ((_fromFrameObj!==undefined && _fromFrameObj.mask.overflow==="hidden") || _frameObj.mask.overflow==="hidden"))) {
				...
				ftl.add(tpGS.gsap.to(_.m,0.001,{overflow:"hidden"}),0);
				if (_.type==="column") ftl.add(tpGS.gsap.to(_.cbgmask,0.001,{overflow:"hidden"}),0);
				if (_.btrans) {
				...
				if (_.fff)
					ftl.add(tpGS.gsap.fromTo([_.m,_.cbgmask],speed,jQuery.extend(true,{},frommask),jQuery.extend(true,{},mask)),0.001);
				else
					ftl.add(tpGS.gsap.to([_.m,_.cbgmask],speed,jQuery.extend(true,{},mask)),0.001);
			} else {
				if (_.btrans!==undefined) {
					...
					if (_.btrans.rX!==0 || _.btrans.rY!=0) mtrans.transformPerspective=tPE;
		...
		if (_.frames.loop!==undefined) {
			...
			if (!_.frames.loop.timeline.curved) {
				...
				if ((obj.mode!=="trigger" && obj.frame!=="frame_999") || obj.frame!=="frame_999") _.loop.preset.fromTo(_.lp,lsspeed,{'-webkit-filter':loopsfilter, 'filter':loopsfilter, x:0,y:0,z:0, minWidth:(_._incolumn || _._ingroup ? "100%" : _.eow===undefined ? 0 : _.eow), minHeight:(_._incolumn || _._ingroup ? "100%" : _.eoh===undefined ? 0 : _.eoh), scaleX:1, scaleY:1, skewX:0, skewY:0, rotationX:0, rotationY:0,rotationZ:0, transformPerspective:tPE, transformOrigin:lof.originX+" "+lof.originY+" "+lof.originZ, opacity:1},checkLoopSkew({ x:lif.x*_R[id].bw, y:lif.y*_R[id].bw, z:lif.z*_R[id].bw, scaleX:lif.scaleX, skewX:lif.skewX, skewY:lif.skewY, scaleY:lif.scaleY,rotationX:lif.rotationX,rotationY:lif.rotationY,rotationZ:lif.rotationZ, ease:"sine.out", opacity:lif.opacity,'-webkit-filter':loopsfilter, 'filter':loopsfilter}),0);
				_.loop.move.to(_.lp,(_.frames.loop.timeline.yoyo_move ? lspeed/2 : lspeed),{x:lof.x*_R[id].bw, y:lof.y*_R[id].bw, z:lof.z*_R[id].bw, ease:_.frames.loop.timeline.ease});
			} else {
				//CALCULATE EDGES
				...
				_.loop.move.to(_.lp,(_.frames.loop.timeline.yoyo_move ? lspeed/2 : lspeed),{motionPath:motionPath, ease:_.frames.loop.timeline.ease});
			}
...
convertHoverTransform = function (_,el) {
	var a = jQuery.extend(true,{},_.transform),nb,l;
...
convertTransformValues = function(obj) {
	...
	for (var atr in a) {
		...
		if (atr==="originX" || atr==="originY" || atr==="originZ") { // COLLECT ORIGINS
			...
			if (jQuery.isNumeric(a[atr],0)) a[atr] = checkReverse(a[atr],obj.frame.reverse,obj.target,atr,obj.id,obj.id);	// NUMERIC ?
			else
			...
			if (a[atr].indexOf("%")>=0 && jQuery.isNumeric(b = parseInt(a[atr],0))) a[atr]= atr==="x" ? checkReverse((_.eow||0)*b/100,obj.frame.reverse,obj.target,atr,obj.id) : atr==="y" ? checkReverse((_.eoh||0)*b/100,obj.frame.reverse,obj.target,atr,obj.id) : a[atr];
			else {
				a[atr] = a[atr].replace("[","").replace("]","");
				a[atr] = checkReverse(a[atr],obj.frame.reverse,obj.target,atr,obj.id,obj.id);
				switch (a[atr]) {
					case "t": case "top": a[atr] = 0-(_.eoh || 0)- (_.type==="column" ? 0 : _.calcy || 0);break;
					case "b": case "bottom": a[atr] = parh - (_.type==="column" ? 0 : _.calcy || 0);break;
					case "l": case "left":a[atr] = 0-(_.eow || 0)-(_.type==="column" ? 0 : _.calcx || 0);break;
					case "r": case "right":  a[atr] = parw - (_.type==="column" ? 0 : _.calcx || 0);
					break;
					case "m": case "c": case "middle": case "center": a[atr] = atr==="x" ?  checkReverse((parw/2 - (_.type==="column" ? 0 : _.calcx || 0) - (_.eow||0)/2),obj.frame.reverse,obj.target,atr,obj.id) : atr==="y" ? checkReverse((parh/2 - (_.type==="column" ? 0 : (_.calcy||0)) - (_.eoh||0)/2),obj.frame.reverse,obj.target,atr,obj.id) : a[atr];
					break;
				}
...
buildFrameObj=function(_,id) {
	...
	if (_.frame_hover!==undefined || _.svgh!==undefined) {
		...
		if (_ISM && (n.frame_hover.transform.instantClick=='true' || n.frame_hover.transform.instantClick==true)) delete n.frame_hover;
		else {
...
syncFrames = function(_,id) {
	...
	for (var f in _) {
		...
		if (f!=='loop' && f!=='frame_hover') {
			...
			if (_[f].timeline!==undefined && _[f].timeline.usePerspective===false && _[f].transform!==undefined && (_[f].transform.rotationY!==undefined || _[f].transform.rotationX!==undefined || _[f].transform.z!==undefined)) {
	...
	if (ignorePerspective) for (var f in _) if (!_.hasOwnProperty(f) || _[f].transform===undefined) continue; else delete _[f].transform.transformPerspective;
	return _;
},
...
getLayersInSlide = function(slide,cname,nocname) {
	...
	for (var i=0;i<ar.length;i++) if (nocname===undefined || ar[i].className.indexOf(nocname)===-1) ret[ar[i].id] = ar[i];
	return ret;
},

FWS = function(a) {
	a = jQuery.isNumeric(a) ? a : a.toLowerCase();
...
getStyleAtStart = function(L,level,id) {
	...
	var cstyles =  window.getComputedStyle(L[0],null),
		...
		gp = pc!==undefined && (cstyles.fontSize == pc.css("fontSize") &&  FWS(cstyles.fontWeight) == FWS(pc.css("fontWeight")) &&  cstyles.lineHeight == pc.css("lineHeight")) ? true : false,
	...
	if (!d._isnotext) {
		...
		d.letterSpacing = _R.revToResp(gp ? dpc.letterspacing===undefined ? parseInt(pc.css('letterSpacing'),0) || "inherit" : dpc.letterspacing : d.letterspacing===undefined ? parseInt(cstyles.letterSpacing==="normal" ? 0 : cstyles.letterSpacing,0) || "inherit" : d.letterspacing,_R[id].rle);
	...
	d.display = gp ? dpc.display===undefined ? pc.css('display') : dpc.display : d.display===undefined ? cstyles.display : d.display;
	...
	d.zIndex = gp ? dpc.zindex===undefined ? parseInt(pc.css('zIndex'),0) || "inherit" : dpc.zindex : d.zindex===undefined ? parseInt(cstyles.zIndex,0) || "inherit" : d.zindex;

	for (i=0;i<4;i++) {
		d['padding'+HR[i]] = _R.revToResp(d['padding'+hr[i]]===undefined ? parseInt(cstyles['padding'+HR[i]],0) || 0 : d['padding'+hr[i]],_R[id].rle);
		d['margin'+HR[i]] = _R.revToResp(d['margin'+hr[i]]===undefined ? parseInt(cstyles['margin'+HR[i]],0) || 0 : d['margin'+hr[i]],_R[id].rle);
		d['border'+HR[i]+'Width'] = d.borderwidth===undefined ? parseInt(cstyles['border'+HR[i]+'Width'],0) || 0 : d.borderwidth[i];
		d['border'+HR[i]+'Color'] = d.bordercolor===undefined ? cstyles["border-"+hr[i]+"-color"] : d.bordercolor;
		d['border'+CO[i]+'Radius'] = _R.revToResp(d.borderradius===undefined ? cstyles['border'+CO[i]+'Radius'] || 0 : d.borderradius[i],_R[id].rle);
	}
	...
	for (i=0;i<4;i++) {
		...
		if (notNeeded(d['border'+CO[i]+'Radius'],'0px')) delete d['border'+CO[i]+'Radius']; else if (notNeeded(d['border'+CO[i]+'Radius'],"0")) delete d['border'+CO[i]+'Radius'];
	}
...
getLayerResponsiveValues = function(L,id,RSL) {
	...
	if (!_._isnotext) {
		...
		if (_.tstroke!==undefined) ret.textStroke = ""+parseInt(_.tstroke.w,0)+"px "+_.tstroke.c;
	}
...
calcResponsiveLayer = function(obj) {
	...
	var obj = getLayerResponsiveValues(L,id,obj.RSL),bw=responsive==="off" ? 1 :  _R[id].bw,
		objCache;
	...
	var POBJ =  jQuery.extend(true,{},_.OBJUPD.POBJ),
		LPOBJ =   jQuery.extend(true,{},_.OBJUPD.LPOBJ);
	...
	if (clasName.indexOf("rs_splitted_")===-1) {
		...
		if(_._isstatic && _.elementHovered) {
			frams = L.data('frames');
			if(frams && frams.frame_hover && frams.frame_hover.transform) for(prop in S) if(S.hasOwnProperty(prop) && frams.frame_hover.transform.hasOwnProperty(prop)) delete S[prop];
		}


		if (L[0].nodeName=="IFRAME" && _R.gA(L[0],"layertype")==="html") {
			winw = obj.basealign =="slide" ? _R[id].width : _R.iWA(id,slideIndex);
			winh = obj.basealign =="slide" ? _R[id].height : _R.iHE(id); //*(_R[id].keepBPHeight || _R[id].currentRowsHeight>_R[id].gridheight[_R[id].level] ? 1 :_R[id].bh)
			...
			S.height = !jQuery.isNumeric(obj.height) && obj.height.indexOf("%") >=0 ?(_._isstatic && !_._incolumn && !_._ingroup) ? winh * parseInt(obj.height,0)/100 : obj.height : minmaxconvert(obj.height,bw,"auto",winw,"auto");
		}
		...
		if (level!="rekursive") {
			...
			var	swid = !jQuery.isNumeric(obj.width) && obj.width.indexOf("%") >=0 ? (_._isstatic && !_._incolumn && !_._ingroup) ? winw * parseInt(obj.width,0)/100 : obj.width : minmaxconvert(obj.width,bw,"auto",winw,"auto"),
				shei = !jQuery.isNumeric(obj.height) && obj.height.indexOf("%") >=0 ?(_._isstatic && !_._incolumn && !_._ingroup) ? winh * parseInt(obj.height,0)/100 : obj.height : minmaxconvert(obj.height,bw,"auto",winw,"auto"),
				Sr = {
			...
			if (_._incolumn) {
				...
				for (i=0;i<4;i++) if (obj['margin'+HR[i]]!==undefined) POBJ['margin'+HR[i]] = (obj['margin'+HR[i]] * bw) + "px";
				LPOBJ.width = "100%";
				...
				if (_.type==="image") tpGS.gsap.set(_.img,{width:Sr.width});  // Make sure Image Inside Layer has 100% Width to fill Inline Block Elements in Columns
			}
			else
			if (!jQuery.isNumeric(obj.width) && obj.width.indexOf("%") >=0) {
				POBJ.minWidth = _.basealign==="slide" || _._ingroup===true? swid :(_R.iWA(id,slideIndex)*_R[id].bw) * parseInt(swid)/100+"px";
				LPOBJ.width="100%";
				MOBJ.width="100%";
			}

			if (!jQuery.isNumeric(obj.height) && obj.height.indexOf("%") >=0) {
				POBJ.minHeight =  _.basealign==="slide" || _._ingroup===true? shei : (_R.iHE(id)*(_R[id].keepBPHeight || _R[id].currentRowsHeight>_R[id].gridheight[_R[id].level] ? 1 :_R[id].bw))* parseInt(shei)/100+"px";
				LPOBJ.height="100%";
				MOBJ.height="100%";
			}
		...
		if (_.type==="row") for (i=0;i<4;i++) if (obj['margin'+HR[i]]!==undefined) POBJ['padding'+HR[i]] = (obj['margin'+HR[i]] * bw) + "px";
		...
		if (_.type==="column" && _.cbg && _.cbg.length>0) {
			...
			a = JSON.stringify(S);
			if (a!==_R[id].emptyObject && a!==_.caches.cbgS) tpGS.gsap.set(_.cbg,S);_.caches.cbgS = a;

			S = {};
			for (i=0;i<4;i++) if (objCache['margin'+HR[i]]) S[hr[i]] = (objCache['margin'+HR[i]] * bw) + "px";
			a = JSON.stringify(S);
			if (a!==_R[id].emptyObject && a!==_.caches.cbgmaskS) { tpGS.gsap.set(_.cbgmask,S);_.caches.cbgmaskS = a;}
		}
		...
		if (_.imgOBJ!==undefined && (_.caches.imgOBJ===undefined || _.caches.imgOBJ.width !==_.imgOBJ.width || _.caches.imgOBJ.height !==_.imgOBJ.height || _.caches.imgOBJ.left !==_.imgOBJ.left || _.caches.imgOBJ.right !==_.imgOBJ.right || _.caches.imgOBJ.top !==_.imgOBJ.top || _.caches.imgOBJ.bottom !==_.imgOBJ.bottom)) {
			_.caches.imgOBJ = jQuery.extend(true,{},_.imgOBJ);
		...
		if (_.mediaOBJ!==undefined && (_.caches.mediaOBJ===undefined || _.caches.mediaOBJ.width !==_.mediaOBJ.width || _.caches.mediaOBJ.height !==_.mediaOBJ.height || _.caches.mediaOBJ.display!==_.mediaOBJ.display)) {
			_.caches.mediaOBJ = jQuery.extend(true,{},_.mediaOBJ);
			_.media.css(_.mediaOBJ);
		}

		if (a!=_R[id].emptyObject && a!=_.caches.LOBJ)  {tpGS.gsap.set(L,LOBJ);_.caches.LOBJ = a;}
		if (b!=_R[id].emptyObject && b!=_.caches.LPOBJ) {tpGS.gsap.set(_.lp,LPOBJ);_.caches.LPOBJ = b;}
		if (c!=_R[id].emptyObject && c!=_.caches.MOBJ)  {tpGS.gsap.set(_.m,MOBJ);_.caches.MOBJ = c;}
		if (d!=_R[id].emptyObject && d!=_.caches.POBJ)  {tpGS.gsap.set(_.p,POBJ);_.caches.POBJ = d; _.caches.POBJ_LEFT = POBJ.left; _.caches.POBJ_TOP = POBJ.top}
