 /********************************************
 * REVOLUTION 6.2.0 EXTENSION - CAROUSEL
 * @version: 6.2.0 (24.01.2020)
...
(function($) {
	...
	jQuery.extend(true,_R, {
		...
		prepareCarousel : function(id,direction,speed) {
			...
			_.slide_offset = _.slide_offset===undefined || !jQuery.isNumeric(_.slide_offset) ? 0 : _.slide_offset;
			...
			if (_.swipeTo===undefined || !jQuery.isNumeric(_.swipeTo)) {
		...
		loadVisibleCarouselItems : function(id,forceload) {
			...
			_R[id].carousel.focused = jQuery.isNumeric(_R[id].carousel.focused) ? _R[id].carousel.focused : 0;
		...
		setCarouselDefaults : function(id,quickmode) {
			var _=_R[id].carousel, loff,roff;

			// IF NOT YET SET CONT WIDHT, WE NEED TO DO SO
			if (_.carouselDefaultsCalled!==true) {
				_R.callContWidthManager(id);
				_R.setSize(id);
				_R.updateDimensions(id);
				_.carouselDefaultsCalled = true;
			}

			if (_R[id].navOutterOffsets===undefined)  _R[id].navOutterOffsets = _R.getHorizontalOffset(_R[id].c,"all");

			if (quickmode!==true) {
				loff = _R[id].navOutterOffsets.left;
				roff = _R[id].navOutterOffsets.right;
			}

			//if (_R[id].conw===undefined)

			// DEFAULT LI WIDTH SHOULD HAVE THE SAME WIDTH OF TH id WIDTH
			_.slide_width = _.stretch!==true ? _R[id].gridwidth[_R[id].level]*(_R[id].bw===0 ? 1 : _R[id].bw) : _R[id].conw;
			_.slide_height = _.stretch!==true ? _R[id].gridheight[_R[id].level]*(_R[id].bw===0 ? 1 : _R[id].bw) : _R[id].conh;
			...
			_.wrapwidth = _R[id].sliderLayout!="auto" ? _.wrapwidth>_R[id].conw ? _R[id].conw : _.wrapwidth : _.wrapwidth>_R[id].width ?_R[id].width : _.wrapwidth;

			if (_.justify===true) {
				_.slide_height = _R[id].gridheight[_R[id].level];
				_.slide_widths = [];
			...
			if (_.quickmode!==true) {
				...
				_.wrapoffset = _.horizontal_align==="center" ? (_R[id].conw-roff - loff - _.wrapwidth)/2 : 0;
				...
				// THE SCREEN WIDTH/2
				_.windhalf = _R[id].sliderLayout==="auto" ? _R[id].width/2 : _R.winW/2;
			}
