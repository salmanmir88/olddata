...
class RevSliderSliderExport extends RevSliderSlider {
	...
	public function __construct(
		...
		$wp_upload_dir			= $this->_frameworkHelper->wp_upload_dir();
	...
	public function export_slider($id = 0){
		...
		if(version_compare($this->get_param(array('settings', 'version')), $this->_frameworkHelper->get_option('revslider_update_version', '6.0.0'), '<')){
	...
	public function modify_used_data(){
		...
		$d = $this->_frameworkHelper->apply_filters('revslider_exportSlider_usedMedia', $d, $this->all_slides, $this->slider_params); //$this->export_slides, $this->static_slide,
	...
	public function add_svg_to_zip(){
		if(!empty($this->used_svg)){
			$c_url	= $this->_frameworkHelper->content_url();
			$c_path	= ABSPATH . 'wp-content';
			$ud		= $this->_frameworkHelper->wp_upload_dir();
	...
	public function add_info_to_zip(){
		if($this->_frameworkHelper->apply_filters('revslider_slider_pack_export', false)){
			if(!$this->usepcl){
