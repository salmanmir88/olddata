...
class RevSliderFolder extends RevSliderSlider {
	...
	public function init_folder_by_id($id){
		...
		$folder = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE `id` = %s AND `type` = 'folder'", $id), Query::ARRAY_A);
	...
	public function get_folders(){
		...
		$folders = array();
		$entries = $wpdb->get_results("SELECT `id` FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE `type` = 'folder'", Query::ARRAY_A);

		if(!empty($entries)){
			foreach($entries as $folder){
				$slider		= new RevSliderFolder($this->_frameworkHelper);
	...
	public function get_folder_by_id($id){
		...
		$folder = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE `type` = 'folder' AND `id` = %s", $id), Query::ARRAY_A);
	...
	public function create_folder($alias = 'New Folder', $parent = 0){
		...
		$title  = $this->_frameworkHelper->esc_html($alias);
		$alias  = $this->_frameworkHelper->sanitize_title($title);
		$temp	= $title;
		$folder = false;
		$ti		= 1;
	...
	public function add_slider_to_folder($children, $folder_id, $replace_all = true){
		$wpdb = $this->_frameworkHelper->getQueryHelper();
		$response	= false;
		$folder		= $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE `id` = %s AND `type` = 'folder'", $folder_id), Query::ARRAY_A);
