...
use \Nwdthemes\Revslider\Model\Revslider\GoogleFonts;
...
class RevSliderPluginUpdate extends RevSliderFunctions {
	...
	public function __construct(
		...
		$this->revision = $this->_frameworkHelper->get_option('revslider_update_version', '6.0.0');
		...
		if(empty($this->googlefonts)){
			$this->googlefonts = GoogleFonts::getFonts();
		}
	...
	public function get_version(){
		$real_version = $this->_frameworkHelper->get_option('revslider_update_version', 1.0);
	...
	public function set_version($set_to){
		...
		$this->_frameworkHelper->update_option('revslider_update_version', $set_to);
	...
	public static function do_update_checks(){
		$upd = new RevSliderPluginUpdate(self::$frameworkHelper);
	...
	public function get_css_navigations(){
		if(empty($this->css_navigations)){
			$css_parser = new RevSliderCssParser($this->_frameworkHelper);
	...
	public function slider_need_update_checks(){
		$finished = $this->_frameworkHelper->get_option('revslider_update_revision_current', '1.0.0');
	...
	public function upgrade_next_slider(){
		$slr = new RevSliderSlider($this->_frameworkHelper);
		...
		$this->_frameworkHelper->update_option('revslider_update_revision_current', $this->revision);
	...
	public function add_v5_styles(){
		...
		foreach($v5 as $v5class){
			$result = $wpdb->get_row($wpdb->prepare("SELECT id FROM " . $wpdb->prefix . RevSliderFront::TABLE_CSS . " WHERE handle = %s", $v5class['handle']), Query::ARRAY_A);
			if(empty($result)){
				//add v5 style
	...
	public function update_css_styles(){
		...
		$styles = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . RevSliderFront::TABLE_CSS, Query::ARRAY_A);
	...
	public function check_settings_table(){
		...
		if($wpdb->get_var("SHOW TABLES LIKE '" . $wpdb->prefix . RevSliderFront::TABLE_SETTINGS . "'") == $wpdb->prefix . RevSliderFront::TABLE_SETTINGS){
			$result = $wpdb->get_row("SELECT `general` FROM " . $wpdb->prefix . RevSliderFront::TABLE_SETTINGS, Query::ARRAY_A);
			if(isset($result['general'])){
				$this->_frameworkHelper->update_option('revslider-global-settings', $result['general']);
			}
		}
	}
				...
				if($template_id > 0){
					...
					foreach($temp_sliders as $t_slider){
						if($t_slider->get_id() === $template_id){
							...
							if($static_id !== false){
								$record = $wpdb->get_row($wpdb->prepare("SELECT * FROM " . $wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES . " WHERE id = %d", $static_id), Query::ARRAY_A);
	...
	public function add_general_settings($sliders = false){
		...
		if(!empty($sliders) && is_array($sliders)){
			$fonts = $this->_frameworkHelper->get_option('tp-google-fonts', array());
				...
				if(!empty($slides) && is_array($slides)){
					foreach($slides as $slide){
						...
						if($ml_id !== false && $ml_id !== $image_id){
							$urlImage = $this->_frameworkHelper->getImagesHelper()->wp_get_attachment_image_src($ml_id, 'full');
								...
								if(isset($layer['type']) && $layer['type'] == 'svg'){
									if(isset($layer['svg']) && isset($layer['svg']->src)){
										...
										if(strpos($layers[$lk]['svg']->src, $path) !== false){
											$layers[$lk]['svg']->src = str_replace($path, Framework::$RS_PLUGIN_URL .'public/assets/assets/svg/', $layers[$lk]['svg']->src);
										}
									}
								}
							}
	...
	public function migrate_slide_to_6_0($slide, $slider = false, $slide_nr = false){
		if($slider === false) $slider = new RevSliderSlider($this->_frameworkHelper);
		$color_picker = new RSColorpicker();
		$ms = array();
	...
	public function migrate_layer_to_6_0($layer, $blank = false, $slide, $slider){
		...
		switch($ml['type']){
			...
			case 'button':
				...
				$ml['media'] = array(
					'imageUrl' => $this->get_val($layer, 'bgimage_url', Framework::$RS_PLUGIN_URL.'admin/assets/images/transparent_placeholder.png'),
					//'imageId' => $img_id,
			...
			case 'image':
				...
				if($img_id === false) $img_id = '';
				$ml['media'] = array(
					'imageUrl' => $this->get_val($layer, 'image_url', Framework::$RS_PLUGIN_URL.'admin/assets/images/transparent_placeholder.png'),
					'imageId' => $img_id,
					'imageFromStream' => false,
					'loaded' => false
				);
				...
				if($this->get_val($layer, 'scaleX', false) !== false){
					...
					if($get_dim === true){
						...
						if($cur_img !== ''){
							...
							$img_data = $this->_frameworkHelper->wp_get_attachment_metadata($cur_id);
	...
	private function change_global_addon_settings_to_6_0(){
			...
			// 404 pages
			if($this->_frameworkHelper->is_plugin_active('revslider-404-addon/revslider-404-addon.php')){

				$settings = $this->_frameworkHelper->get_option('revslider_404_addon');
				if($settings !== false){
					$options = array();
					parse_str($settings, $options);
					if(isset($options['revslider-404-addon-active']) && intval($options['revslider-404-addon-active'])){
						$this->_frameworkHelper->update_option('revslider_404_enabled', 1);
					}
				}
			...
			if($this->_frameworkHelper->is_plugin_active('revslider-maintenance-addon/revslider-maintenance-addon.php')){

				$settings = $this->_frameworkHelper->get_option('revslider_maintenance_addon');
				if($settings !== false){
					$options = array();
					parse_str($settings, $options);
					if(isset($options['revslider-maintenance-addon-active']) && intval($options['revslider-maintenance-addon-active'])){
						$this->_frameworkHelper->update_option('revslider_maintenance_enabled', 1);
					}
				}
			...
			if($this->_frameworkHelper->is_plugin_active('revslider-backup-addon/revslider-backup-addon.php')){
				$this->_frameworkHelper->update_option('revslider_backup_enabled', 1);
			}

			// featured slider
			if($this->_frameworkHelper->is_plugin_active('revslider-featured-addon/revslider-featured-addon.php')){
				$this->_frameworkHelper->update_option('revslider_featured_enabled', 1);
			}

			// wp gallery
			if($this->_frameworkHelper->is_plugin_active('revslider-gallery-addon/revslider-gallery-addon.php')){
				$this->_frameworkHelper->update_option('revslider_gallery_enabled', 1);
			}

			// login
			if($this->_frameworkHelper->is_plugin_active('revslider-login-addon/revslider-login-addon.php')){
				$this->_frameworkHelper->update_option('revslider_login_enabled', 1);
			}

			// social sharing
			if($this->_frameworkHelper->is_plugin_active('revslider-sharing-addon/revslider-sharing-addon')){
				$this->_frameworkHelper->update_option('revslider_sharing_enabled', 1);
			}

			// related posts slider
			if($this->_frameworkHelper->is_plugin_active('revslider-rel-posts-addon/revslider-rel-posts-addon.php')){
				$this->_frameworkHelper->update_option('revslider_rel_posts_enabled', 1);
			}

			// prev/next slider
			if($this->_frameworkHelper->is_plugin_active('revslider-prevnext-posts-addon/revslider-prevnext-posts-addon.php')){
				$this->_frameworkHelper->update_option('revslider_prevnext_posts_enabled', 1);
			}
	...
	public function change_global_settings_to_6_2_0(){
		$global = $this->_frameworkHelper->maybe_unserialize($this->_frameworkHelper->get_option('revslider-global-settings', '')); //get the old structure as serialized
		...
		if($_->get_param('wb_enable', false) !== false){
			$obj['revslider-whiteboard-addon'] = array(
				...
				'writehand' => array(
					...
					'source' => ($_->get_param('wb_writehand_source') === '1') ? Framework::$WP_PLUGIN_URL .'/revslider-whiteboard-addon/'.'assets/images/write_right_angle.png' : $_->get_param('wb_writehand_source_custom', Framework::$WP_PLUGIN_URL .'/revslider-whiteboard-addon/'.'assets/images/write_right_angle.png'),
				),
				'movehand' => array(
					...
					'source' => ($_->get_param('wb_movehand_source') === '1') ? Framework::$WP_PLUGIN_URL .'/revslider-whiteboard-addon/'.'assets/images/hand_point_right.png' : $_->get_param('wb_movehand_source_custom', Framework::$WP_PLUGIN_URL .'/revslider-whiteboard-addon/'.'assets/images/hand_point_right.png'),
				),
			);
		}
	...
	*/
	...
	public function _simplify_slides($_){
		$_slide = new RevSliderSlide($this->_frameworkHelper);
