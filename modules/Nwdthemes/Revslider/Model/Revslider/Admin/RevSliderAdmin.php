...
class RevSliderAdmin extends RevSliderFunctionsAdmin {
	...
	public function enqueue_admin_styles(){
		$this->_frameworkHelper->wp_enqueue_style('rs-open-sans', '//fonts.googleapis.com/css?family=Open+Sans:400,300,700,600,800');
		//$this->_frameworkHelper->wp_enqueue_style('revslider-global-styles', Framework::$RS_PLUGIN_URL . 'admin/assets/css/global.css', array(), Framework::RS_REVISION);
		$this->_frameworkHelper->wp_enqueue_style(array('wp-jquery-ui', 'wp-jquery-ui-core', 'wp-jquery-ui-dialog', 'wp-color-picker'));

		if(in_array($this->get_val(Data::$_GET, 'page'), $this->pages)){
			//$this->_frameworkHelper->wp_enqueue_style('codemirror-css', Framework::$RS_PLUGIN_URL .'admin/assets/css/codemirror.css', array(), Framework::RS_REVISION);
			if($this->_frameworkHelper->is_rtl()){
				$this->_frameworkHelper->wp_enqueue_style('rs-new-plugin-settings-rtl', Framework::$RS_PLUGIN_URL . 'admin/assets/css/builder-rtl.css', array('rs-new-plugin-settings'), Framework::RS_REVISION);
			}
		}
	}
	...
	public function enqueue_admin_scripts(){
		...
		$this->_frameworkHelper->wp_enqueue_script(array('jquery', 'jquery-ui-core', 'jquery-ui-mouse', 'jquery-ui-accordion', 'jquery-ui-datepicker', 'jquery-ui-dialog', 'jquery-ui-slider', 'jquery-ui-autocomplete', 'jquery-ui-sortable', 'jquery-ui-droppable', 'jquery-ui-tabs', 'jquery-ui-widget', 'wp-color-picker', 'wpdialogs', 'updates'));
		$this->_frameworkHelper->wp_enqueue_script(array('wp-color-picker'));

		if(in_array($this->get_val(Data::$_GET, 'page'), $this->pages)){

			$view = $this->get_val(Data::$_GET, 'view');

			$this->_frameworkHelper->wp_localize_script('revbuilder-admin', 'RVS_LANG', $this->get_javascript_multilanguage()); //Load multilanguage for JavaScript
		}
	...
	protected static function add_media_upload_includes(){
		...
		self::$frameworkHelper->wp_enqueue_script('thickbox');
		self::$frameworkHelper->wp_enqueue_script('media-upload');
		self::$frameworkHelper->wp_enqueue_style('thickbox');
	}
	...
	public function load_plugin_textdomain(){
		$this->_frameworkHelper->load_plugin_textdomain('revslider', false, dirname(Framework::RS_PLUGIN_SLUG_PATH) . '/languages/');
		$this->_frameworkHelper->load_plugin_textdomain('revsliderhelp', false, dirname(Framework::RS_PLUGIN_SLUG_PATH) . '/languages/');
	}
	...
	public function add_admin_pages(){
		...
		$this->screens[] = $this->_frameworkHelper->add_menu_page('Slider Revolution', 'Slider Revolution', $role, 'revslider', array($this, 'display_admin_page'), 'dashicons-update');
	}
	...
	public static function custom_post_fields_output(){
		...
		$meta = self::$frameworkHelper->get_post_meta(get_the_ID(), 'slide_template', true);
		$meta = ($meta == '') ? 'default' : $meta;

		$page_bg = self::$frameworkHelper->get_post_meta(get_the_ID(), 'rs_page_bg_color', true);
		...
		$latest_version	= self::$frameworkHelper->get_option('revslider-latest-version', RS_REVISION);
			...
			<li id="slide_template_row">
					<label class="rs_wp_ppset" id="slide_template_text"><?php _e('Slide Template', 'revslider');?></label><select style="max-width:82px" name="slide_template" id="slide_template">
							...
							foreach($output as $handle => $name){
								echo '<option ' . self::$frameworkHelper->selected($handle, $meta) . ' value="' . $handle . '">' . $name . '</option>';
							}
							?></select>
			</li>
	...
	public static function on_save_post(){
		...
		self::$frameworkHelper->update_post_meta($post_id, 'slide_template', $slide_template);

		// Blank Page Template Background Color
		$rs_page_bg_color = $f->get_post_var('rs_page_bg_color');
		self::$frameworkHelper->update_post_meta($post_id, 'rs_page_bg_color', $rs_page_bg_color);

		// Set/Unset Blank Template depending on Blank Template Switch
		$rs_blank_template = $f->get_post_var('rs_blank_template');
		if( empty( $rs_blank_template ) && !empty($rs_page_bg_color)  && self::$frameworkHelper->get_post_meta($post_id, '_wp_page_template',true) == '../public/views/revslider-page-template.php' ){
			self::$frameworkHelper->update_post_meta($post_id, '_wp_page_template','');
		}
		if( !empty( $rs_blank_template ) &&  $rs_blank_template=="on" ) {
			self::$frameworkHelper->update_post_meta($post_id, '_wp_page_template','../public/views/revslider-page-template.php');
		}
	}
	...
	private function do_update_checks(){
		$upgrade	= new RevSliderUpdate($this->_frameworkHelper, Framework::RS_REVISION);
		$library	= new RevSliderObjectLibrary($this->_frameworkHelper);
		$template	= new RevSliderTemplate($this->_frameworkHelper);
		$validated	= $this->_frameworkHelper->get_option('revslider-valid', 'false');
		$stablev	= $this->_frameworkHelper->get_option('revslider-stable-version', '0');
	...
	public function add_actions(){
		...
		$this->_frameworkHelper->add_action('plugins_loaded', array($this, 'load_plugin_textdomain'));
		$this->_frameworkHelper->add_action('admin_head', array($this, 'hide_notices'), 1);
		$this->_frameworkHelper->add_action('admin_menu', array($this, 'add_admin_pages'));
		$this->_frameworkHelper->add_action('add_meta_boxes', array($this, 'add_slider_meta_box'));
		$this->_frameworkHelper->add_action('save_post', array($this, 'on_save_post'));
		$this->_frameworkHelper->add_action('admin_enqueue_scripts', array($this, 'enqueue_admin_styles'));
		$this->_frameworkHelper->add_action('admin_enqueue_scripts', array($this, 'enqueue_admin_scripts'));
		$this->_frameworkHelper->add_action('wp_ajax_revslider_ajax_action', array($this, 'do_ajax_action')); //ajax response to save slider options.
		$this->_frameworkHelper->add_action('wp_ajax_revslider_ajax_call_front', array($this, 'do_front_ajax_action'));
		$this->_frameworkHelper->add_action('wp_ajax_nopriv_revslider_ajax_call_front', array($this, 'do_front_ajax_action')); //for not logged in users

		if(isset($pagenow) && $pagenow == 'plugins.php'){
			$this->_frameworkHelper->add_action('admin_notices', array($this, 'add_plugins_page_notices'));
		}

		$this->_frameworkHelper->add_action('admin_init', array($this, 'merge_addon_notices'), 99);
		$this->_frameworkHelper->add_action('admin_init', array($this, 'add_suggested_privacy_content'), 15);
	}
	...
	public function add_filters(){
		$this->_frameworkHelper->add_filter('admin_body_class', array($this, 'modify_admin_body_class'));

		$this->_frameworkHelper->add_filter('plugin_locale', array($this, 'change_lang'), 10, 2);
	}
	...
	public function merge_addon_notices(){
		...
		if(Framework::$revslider_addon_notice_merged > 0){
			$this->_frameworkHelper->add_action('admin_notices', array($this, 'add_addon_plugins_page_notices'));
		}
	}
	...
	public static function add_plugins_page_notices(){
		$plugins = self::$frameworkHelper->get_plugins();
		...
		foreach($plugins as $plugin_id => $plugin){
			...
			$add = (self::$frameworkHelper->get_option('revslider-valid', 'false') == 'false' || version_compare(self::$frameworkHelper->get_option('revslider-latest-version', Framework::RS_REVISION), $plugin['Version'], '>')) ? true : false;

			if($add){
				self::$frameworkHelper->add_action('after_plugin_row_' . $plugin_id, array('\Nwdthemes\Revslider\Model\Revslider\Admin\RevSliderAdmin', 'add_notice_wrap_pre'), 10, 3);
			}

			//check version, latest updates and if registered or not
			if(self::$frameworkHelper->get_option('revslider-valid', 'false') == 'false'){
				//activate for updates and support
				self::$frameworkHelper->add_action('after_plugin_row_' . $plugin_id, array('\Nwdthemes\Revslider\Model\Revslider\Admin\RevSliderAdmin', 'show_purchase_notice'), 10, 3);
			}

			if($add){
				self::$frameworkHelper->add_action('after_plugin_row_' . $plugin_id, array('\Nwdthemes\Revslider\Model\Revslider\Admin\RevSliderAdmin', 'add_notice_wrap_post'), 10, 3);
			}
		}
	}
	...
	public static function add_notice_wrap_pre($plugin_file, $plugin_data, $plugin_status){
		$wp_list_table = self::$frameworkHelper->_get_list_table('WP_Plugins_List_Table');
		$slug = dirname($plugin_file);
		if(is_network_admin()){
			$active_class = is_plugin_active_for_network($plugin_file) ? ' active' : '';
		}else{
			$active_class = self::$frameworkHelper->is_plugin_active($plugin_file) ? ' active' : '';
		}
	...
	public static function show_purchase_notice($plugin_file, $plugin_data, $plugin_status){
		?>
		<p>
			<?php echo __('Activate Slider Revolution for <a href="https://revolution.themepunch.com/direct-customer-benefits/" target="_blank">Premium Benefits (e.g. Live Updates)</a>.', 'revslider');?>
		</p>
        <?php
	}
	...
	public function do_ajax_action(){
		@ini_set('memory_limit', $this->_frameworkHelper->apply_filters('admin_memory_limit', Framework::WP_MAX_MEMORY_LIMIT));
		...
		try{
			...
			if(!$this->_frameworkHelper->current_user_can('administrator') && $this->_frameworkHelper->apply_filters('revslider_restrict_role', true)){
				switch($action){
					...
					break;
					default:
						$return = $this->_frameworkHelper->apply_filters('revslider_admin_onAjaxAction_user_restriction', true, $action, $data, $slider, $slide, $operations);
						if($return !== true){
							$this->ajax_response_error(__('Function Only Available for Adminstrators', 'revslider'));
							exit;
						}
			...
			if($this->_frameworkHelper->wp_verify_nonce($nonce, 'revslider_actions') == false){
			...
			switch($action){
				case 'activate_plugin':
					...
					$rs_license = new RevSliderLicense($this->_frameworkHelper);
				...
				case 'dismiss_dynamic_notice':
					...
					$notices_discarded = $this->_frameworkHelper->get_option('revslider-notices-dc', array());
					if(!empty($ids)){
						foreach($ids as $_id){
							$notices_discarded[] = $this->_frameworkHelper->esc_attr(trim($_id));
						}

						$this->_frameworkHelper->update_option('revslider-notices-dc', $notices_discarded);
					}

					$this->ajax_response_success(__('Saved', 'revslider'));
				break;
				case 'check_for_updates':
					$update = new RevSliderUpdate($this->_frameworkHelper, Framework::RS_REVISION);
					$update->force = true;

					$update->_retrieve_version_info();
					$version = $this->_frameworkHelper->get_option('revslider-latest-version', Framework::RS_REVISION);
				...
				break;
				case 'get_template_information_short':
					$templates = new RevSliderTemplate($this->_frameworkHelper);
				...
				case 'import_template_slider': //before: import_slider_template_slidersview
					...
					if($filepath !== false){
						...
						$slider = new RevSliderSliderImport($this->_frameworkHelper);
						...
						if($this->get_val($return, 'success') == true){
							...
							if(intval($new_id) > 0){
								...
								if(intval($folder_id) > 0){
									$folder = new RevSliderFolder($this->_frameworkHelper);
									$folder->add_slider_to_folder($new_id, $folder_id, false);
								}

								$new_slider = new RevSliderSlider($this->_frameworkHelper);
				...
				break;
				case 'import_slider':
					$import = new RevSliderSliderImport($this->_frameworkHelper);
				...
				break;
				case 'export_slider':
					$export = new RevSliderSliderExport($this->_frameworkHelper);
				...
				case 'export_slider_html':
					$export = new RevSliderSliderExportHtml($this->_frameworkHelper);
					$id = intval($this->get_request_var('id'));
				...
				case 'get_list_of':
					...
					switch($type){
						...
						case 'posttypes':
							...
							$output = 'objects';
							$operator = 'and';
							$post_types = $this->_frameworkHelper->get_post_types($args, $output, $operator);
							...
							foreach($post_types as $post_type){
								...
								if(!in_array($post_type->name, array('post', 'page', 'attachment', 'revision', 'nav_menu_item', 'custom_css', 'custom_changeset', 'user_request'))){
									$taxonomy_objects = $this->_frameworkHelper->get_object_taxonomies($post_type->name, 'objects');
				...
				case 'load_wordpress_object':
					...
					$file = $this->_frameworkHelper->wp_get_attachment_image_src($id, $type);
					if($file !== false){
				...
				break;
				case 'create_navigation_preset':
					$nav = new RevSliderNavigation($this->_frameworkHelper);
				...
				case 'save_navigation': //also deletes if requested
					$_nav = new RevSliderNavigation($this->_frameworkHelper);
				...
				case 'delete_animation':
					...
					$admin = new RevSliderFunctionsAdmin($this->_frameworkHelper);
				...
				case 'preview_slider':
					...
					Framework::$revslider_is_preview_mode = true;
					$rev_slider_front = new RevSliderFront($this->_frameworkHelper);

					$html = $this->_frameworkHelper->generatePreview(get_defined_vars());
				...
				case 'check_system':
					...
					$fun = new RevSliderFunctionsAdmin($this->_frameworkHelper);
				...
				case 'set_favorite':
					...
					$id = $this->_frameworkHelper->esc_attr($this->get_val($data, 'id'));
				...
				break;
				case 'get_addon_list':
					$addon = new RevSliderAddons($this->_frameworkHelper);
					$addons = $addon->get_addon_list();

					$this->_frameworkHelper->update_option('rs-addons-counter', 0); //set the counter back to 0
				...
				case 'activate_addon':
					...
					if($return === true){
						...
						$data = array();
						$data = $this->_frameworkHelper->apply_filters('revslider_activate_addon', $data, $handle);
				...
				case 'create_draft_page':
					...
					if ($page_id) {
						$response['open'] = $this->_frameworkHelper->getPageUrl($page_id);
						$response['edit'] = $this->_frameworkHelper->getEditPageUrl($page_id);
					}
				...
				case 'export_layer_group': //developer function only :)
					...
					$export = new RevSliderSliderExport($this->_frameworkHelper, $title);
				...
				case 'load_library_image':
					...
					if(!empty($images)){
						$templates = new RevSliderTemplate($this->_frameworkHelper);
						$obj = new RevSliderObjectLibrary($this->_frameworkHelper);
				...
				break;
				case 'get_help_directory':
					$help_data = RevSliderHelp::getIndex($this->_frameworkHelper);
				...
				break;
				case 'set_tooltip_preference':
					$this->_frameworkHelper->update_option('revslider_hide_tooltips', true);
					$return = 'Preference Updated';
				break;
				case 'save_color_preset':
					$presets = $this->get_val($data, 'presets', array());
					$color_presets = RSColorpicker::save_color_presets($this->_frameworkHelper, $presets);
					$this->ajax_response_data(array('presets' => $color_presets));
				break;
				case 'get_facebook_photosets':
					if(!empty($data['url'])){
						$facebook = new RevSliderFacebook($this->_frameworkHelper);
				...
				case 'get_flickr_photosets':
					...
					if(!empty($data['url']) && !empty($data['key'])){
						$flickr = new RevSliderFlickr($this->_frameworkHelper, $data['key']);
				...
				case 'get_youtube_playlists':
					if(!empty($data['id'])){
						$youtube = new RevSliderYoutube($this->_frameworkHelper, trim($data['api']), trim($data['id']));
				...
				break;
				case 'fix_database_issues':
					$this->_frameworkHelper->update_option('revslider_table_version', '1.0.0');
				...
				break;
				default:
					$return = ''; //''is not allowed to be added directly in $this->_frameworkHelper->apply_filters(), so its needed like this
					$return = $this->_frameworkHelper->apply_filters('revslider_do_ajax', $return, $action, $data);
					if($return){
						if(is_array($return)){
	...
	public function do_front_ajax_action(){
		...
		//verify the token
		$is_verified = $this->_frameworkHelper->wp_verify_nonce($token, 'RevSlider_Front');
		...
		if($is_verified){
			...
			switch($this->get_post_var('client_action', false)){
				case 'get_slider_html':
					...
					if($alias !== ''){
						$sr = new RevSliderSlider($this->_frameworkHelper);
					...
					if($id > 0){
						$html = '';
						ob_start();
						$slider = new RevSliderOutput($this->_frameworkHelper);
	...
	public function show_error($view, $message){
		...
		echo $this->_frameworkHelper->esc_attr($view);
		echo ' - Error: <span>';
		echo $this->_frameworkHelper->esc_attr($message);
		echo '</span>';
		echo '</div>';
		exit;
	}
	...
	public function validate_filepath($filepath, $prefix = null){
		...
		$message = $prefix.' '.$this->_frameworkHelper->esc_attr($filepath).' not exists!';
	...
	public function create_fake_post($content, $title = 'Slider Revolution'){
		...
		$post->post_date	 = $this->_frameworkHelper->current_time('mysql');
		$post->post_date_gmt = $this->_frameworkHelper->current_time('mysql', 1);
	...
	public static function esc_js_deep($value){
		$value = is_array($value) ? array_map(array('\Nwdthemes\Revslider\Model\Revslider\Admin\RevSliderAdmin', 'esc_js_deep'), $value) : self::$frameworkHelper->esc_js($value);
