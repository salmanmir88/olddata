...
class RevSliderSlider extends RevSliderFunctions {
	...
	/**
	 * old version of $this->_frameworkHelper->get_param();
	...
	public function importSliderFromPost($update_animation = true, $updateStatic = true, $exact_filepath = false, $is_template = false, $single_slide = false, $update_navigation = true, $install = true){
		$i = new RevSliderSliderImport($this->_frameworkHelper);
	...
	public function init_by_id($sid, $show_error = true){
		...
		$slider_data = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE id = %d", $sid), Query::ARRAY_A);
		if(empty($slider_data) && !$this->_frameworkHelper->is_admin() && $show_error === true){
	...
	public function init_by_alias($alias, $show_error = true){
		...
		$slider_data = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE alias = %s", $alias), Query::ARRAY_A);
		if(empty($slider_data)){
			$alias = str_replace('-', ' ', $alias); //go back to an very old option where an slider alias could have a space
			$slider_data = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE alias = %s", $alias), Query::ARRAY_A);
		}
		if(empty($slider_data) && !$this->_frameworkHelper->is_admin() && $show_error === true){
			throw new \Exception('Slider with alias '.$this->_frameworkHelper->sanitize_text_field($this->_frameworkHelper->esc_attr($alias)).' not found.');
		}
	...
	public function init_by_data($data){
		$data = $this->_frameworkHelper->apply_filters('revslider_slider_init_by_data', $data);
		...
		$this->_frameworkHelper->do_action('revslider_slider_init_by_data_post', $this);
	}
	...
	public function get_shortcode(){
		return '{{block class="Nwdthemes\Revslider\Block\Revslider" alias="'.$this->alias.'"}}';
	}
	...
	public function get_slider_count(){
		...
		$count = count($wpdb->get_results("SELECT COUNT(*) FROM ".$wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE `type` = '' OR `type` IS NULL", Query::ARRAY_A));
	...
	public function get_first_slide_id_from_gallery(){
		...
		$slides = array();
		$record = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE slider_id = %s ORDER BY slide_order ASC LIMIT 0,1", array($this->get_id())), Query::ARRAY_A);
	...
	public function get_alias_by_id($slider_id){
		...
		$record = $wpdb->get_row($wpdb->prepare("SELECT `alias` FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE id = %s LIMIT 0,1", array($slider_id)), Query::ARRAY_A);
				...
				if($image != ''){
					...
					switch($altOption){
						case 'media_library';
							...
							if($id > 0) $alt = $this->_frameworkHelper->get_post_meta($id, '_wp_attachment_image_alt', true);
						break;
						case 'file_name';
							$alt = $image;
						break;
					...
					switch($titleOption){
						case 'media_library';
							$id = attachment_url_to_postid($image);
							if($id > 0) $title = $this->_frameworkHelper->get_the_title($id);
						break;
						case 'file_name';
							$title = $image;
						break;
	...
	public static function alias_exists($alias, $return_id = false){
		...
		$alias_exists = $wpdb->get_row($wpdb->prepare("SELECT id FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE alias = %s", $alias), Query::ARRAY_A);
	...
	public function delete_all_slides(){
		...
		$this->_frameworkHelper->do_action('revslider_slider_delete_all_slides', $this->id);
		$this->_frameworkHelper->do_action('revslider_slider_deleteAllSlides', $this->id);
	}
	...
	public function delete_static_slide(){
		...
		$this->_frameworkHelper->do_action('revslider_slider_delete_static_slide', $this->id);
	}
	...
	public function duplicate_slider_by_id($id, $is_template = false){
		...
		if($is_template){
			...
			$talias	= strtolower($this->_frameworkHelper->sanitize_title_with_dashes($title));
		}else{
		...
		while($this->alias_exists($talias)){ //set a new alias and title if its existing in database
			$talias = strtolower($this->_frameworkHelper->sanitize_title_with_dashes($title)) .  '-' . $ti;
			$ti++;
		}
	...
	public function update_title($new_title){
		...
		$new_title = stripslashes($this->_frameworkHelper->esc_html($new_title));
		if(!empty($new_title)){
	...
	public function update_slider_tags($slider_id, $tags){
		...
		$record	  = $wpdb->get_row($wpdb->prepare("SELECT `settings` FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE id = %s", $slider_id), Query::ARRAY_A);
		$cur_tags = array();
	...
	public function get_last_slider_id(){
		...
		$record = $wpdb->get_row("SELECT `id` FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." ORDER BY `id` DESC LIMIT 0,1", Query::ARRAY_A);
	...
	private function duplicate_slider($title = false, $prefix = false){
		...
		if($title === false){
			...
			$new_title = ($prefix !== false) ? $this->_frameworkHelper->sanitize_text_field($title.' '.$this->get_val($params, 'title')) : $this->_frameworkHelper->sanitize_text_field($title);
			$new_alias = ($prefix !== false) ? $this->_frameworkHelper->sanitize_title($title.' '.$this->get_val($params, 'title')) : $this->_frameworkHelper->sanitize_title($title);
		...
		$params['shortcode'] = '{{block class="Nwdthemes\Revslider\Block\Revslider" alias="'. $new_alias .'"}}';
		...
		$slides_ids = array();
		$slides = $wpdb->get_results($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE slider_id = %s", $this->id), Query::ARRAY_A);
		if(!empty($slides)){
		...
		$static_id	= 0;
		if($staticID !== false){
			$record = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES ." WHERE id = %s", $staticID), Query::ARRAY_A);
			unset($record['id']);
		...
		//update actions
		$slides = $wpdb->get_results($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE slider_id = %s", $slider_last_id), Query::ARRAY_A);
		if($static_id > 0){
			$slides_static = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES ." WHERE id = %s", $static_id), Query::ARRAY_A);
			if(!empty($slides_static)) $slides[] = $slides_static;
		}

		if(!empty($slides)){
			foreach($slides as $slide){
				$c_slide	= new RevSliderSlide($this->_frameworkHelper);
		...
		$c_slider = new RevSliderSliderImport($this->_frameworkHelper);
	...
	public function check_alias($alias){
		...
		$slider	= $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE alias = %s", $alias).$add, Query::ARRAY_A);
	...
	public function create_blank_slider(){
		...
		$slider_data = array(
			...
			'settings'	=> json_encode(array('version' => Framework::RS_REVISION), JSON_FORCE_OBJECT),
			'type'		=> ''
		);
	...
	public function save_slider($slider_id, $data){
		...
		$title	= $this->_frameworkHelper->sanitize_text_field($this->get_val($params, 'title'));
		$alias	= $this->_frameworkHelper->sanitize_text_field($this->get_val($params, 'alias'));
		...
		$slider_data = $this->_frameworkHelper->apply_filters('revslider_slider_saveData', $slider_data);
	...
	public function get_sliders($templates = false){
		...
		$slider_data = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$wpdb->prefix . RevSliderFront::TABLE_SLIDER." WHERE `type` != 'folder' ORDER BY %s %s", array($do_order, $direction)), Query::ARRAY_A); //WHERE `type` = '' OR `type` IS NULL
		if(!empty($slider_data)){
	...
	public function get_slides_for_export(){
		...
		return $this->_frameworkHelper->apply_filters('revslider_get_slides_for_export', $this->_frameworkHelper->apply_filters('revslider_getSlidesForExport', $export));
	}
	...
	public function get_static_slide_for_export(){
		...
		return $this->_frameworkHelper->apply_filters('revslider_getStaticSlideForExport', $static_slide);
	}
	...
	public function get_parent_slides($published, $gal_ids, $lang){
		...
		$this->_frameworkHelper->apply_filters('revslider_get_parent_slides_pre', $lang, $published, $gal_ids, $this);
		...
		$this->_frameworkHelper->apply_filters('revslider_get_parent_slides_post', $parent_slides, $published, $gal_ids, $this);
	...
	public function get_max_order(){
		...
		$record = $wpdb->get_row($wpdb->prepare("SELECT slide_order FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE slider_id = %d ORDER BY slide_order DESC LIMIT 0,1", $this->id), Query::ARRAY_A);
	...
	public function copy_slide_to_slider($data){
		...
		$add_to_slider	= $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE id = %s", $slider_id), Query::ARRAY_A); //check if ID exists

		if(empty($add_to_slider))
			return __('Slide could not be duplicated', 'revslider');

		//get last slide in slider for the order
		$slide_order	= $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE slider_id = %s ORDER BY slide_order DESC", $slider_id), Query::ARRAY_A);
		$slide_to_copy	= $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE id = %s", $slide_id), Query::ARRAY_A);
	...
	public function get_used_fonts($full = false){
		$gf			= array();
		$sl			= new RevSliderSlide($this->_frameworkHelper);
		$mslides	= $this->get_slides(true);
		$static_id	= $sl->get_static_slide_id($this->get_id());

		if($static_id !== false){
			$msl		= new RevSliderSlide($this->_frameworkHelper);
		...
		return $this->_frameworkHelper->apply_filters('revslider_getUsedFonts', $gf);
	}
		...
		switch($source){
			...
			case 'post':
				...
				if($subtype === 'current_post'){
					$currentProductId = $this->_frameworkHelper->getCurrentProductId();
					$posts = $this->get_specific_posts(array($currentProductId));
	...
	public function get_related_posts(){
		...
		$get_relateds		= $this->_frameworkHelper->apply_filters('revslider_get_related_posts', $query, $post_id);
	...
	public function get_slides_from_stream($published = false){
		...
		switch($sourcetype){
			case 'facebook':
				$facebook = new RevSliderFacebook($this->_frameworkHelper, $this->get_param(array('source', 'facebook', 'transient'), '1200'));
			...
			break;
			case 'twitter':
				$twitter	 = new RevSliderTwitter($this->_frameworkHelper, $this->get_param(array('source', 'twitter', 'consumerKey')), $this->get_param(array('source', 'twitter', 'consumerSecret')), $this->get_param(array('source', 'twitter', 'accessToken')), $this->get_param(array('source', 'twitter', 'accessSecret')), $this->get_param(array('source', 'twitter', 'transient'), '1200'));
			...
			break;
			case 'instagram':
				$instagram	 = new RevSliderInstagram($this->_frameworkHelper, $this->get_param(array('source', 'instagram', 'transient'), '1200'));
			...
			break;
			case 'flickr':
				$flickr = new RevSliderFlickr($this->_frameworkHelper, $this->get_param(array('source', 'flickr', 'apiKey')), $this->get_param(array('source', 'flickr', 'transient'), '1200'));
			...
			case 'youtube':
				...
				$youtube	 = new RevSliderYoutube($this->_frameworkHelper, $this->get_param(array('source', 'youtube', 'api')), $channel_id, $this->get_param(array('source', 'youtube', 'transient'), '1200'));
			...
			break;
			case 'vimeo':
				$vimeo = new RevSliderVimeo($this->_frameworkHelper, $this->get_param(array('source', 'vimeo', 'transient'), '1200'));
		...
		$posts = $this->_frameworkHelper->apply_filters('revslider_pre_mod_stream_data', $posts, $sourcetype, $this->id);
		...
		$posts = $this->_frameworkHelper->apply_filters('revslider_post_mod_stream_data', $posts, $sourcetype, $this->id);
		...
		foreach($posts as $data){
			...
			$_slides[$i] = new RevSliderSlide($this->_frameworkHelper);
	...
	private function get_posts_by_categories($published = false){
		...
		return $this->_frameworkHelper->apply_filters('revslider_get_posts_by_categories', $post, $this);
	}
	...
	public function get_overview_data($slider = false){
		...
		return array(
			...
			'slide_id'	=> $id,
			'slide_ids'	=> $slide_ids,
			'title'		=> $this->_frameworkHelper->esc_html($slider->get_title()),
			'alias'		=> $slider->get_alias(),
			'source'	=> $this->_frameworkHelper->esc_html($type),
	...
	public function get_specific_posts($gal_ids = array()){
		...
		if(!empty($gal_ids) && $gal_ids[0] !== ''){
			$posts	= $gal_ids;
			$posts	= $this->_frameworkHelper->apply_filters('revslider_set_posts_list_gal', $posts, $this->get_id());
			$is_gal	= true;
		}else{
			if(isset($gal_ids[0])){
				unset($gal_ids[0]);
			...
			$posts = $this->_frameworkHelper->apply_filters('revslider_set_posts_list', $posts, $this->get_id());
		}
	...
	public function get_posts_by_id($ids, $slider_id, $is_gal, $additional = array()){
		...
		$query	= $this->_frameworkHelper->apply_filters('revslider_get_posts', $query, $slider_id);

		$posts= $this->_frameworkHelper->getProductsHelper()->getProductsByQuery($query);
	...
	public function get_posts_by_category($slider_id, $cat_id, $sort_by = 'ID', $direction = 'DESC', $max_posts = -1, $post_types = 'any', $taxonomies = 'category', $addition = array(), $type = ''){
		$a = $this->_frameworkHelper->apply_filters('revslider_get_posts_by_category', array('slider_id' => $slider_id, 'cat_id' => $cat_id, 'sort_by' => $sort_by, 'direction' => $direction, 'max_posts' => $max_posts, 'post_types' => $post_types, 'taxonomies' => $taxonomies, 'addition' => $addition, 'type' => $type), $this);
	...
	public function get_post_categories($post_id, $tax){
		...
		$cats = $this->_frameworkHelper->wp_get_post_terms($post_id, $tax);
