...
class RevSliderData {
	...
	public function __construct(
		...
		self::$frameworkHelper = $this->_frameworkHelper;
	}
	...
	public function get_font_familys(){
		...
		return $this->_frameworkHelper->apply_filters('revslider_data_get_font_familys', $this->_frameworkHelper->apply_filters('revslider_operations_getArrFontFamilys', $fonts));
	}
	 ...
	 **/
	public function get_animations_v5($pre = ''){
		$custom = array();
		$temp = array();
		$sort = array();
	...
	public function fill_animations(){
		if(empty($this->animations)){
			...
			$result = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . RevSliderFront::TABLE_LAYER_ANIMATIONS, Query::ARRAY_A);
			...
			if(!empty($this->animations)){
				array_walk_recursive($this->animations, array($this, 'force_to_boolean'));
			}
		}
	}
	...
	public function fill_css(){
		if(empty($this->css)){
			...
			$css_data = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . RevSliderFront::TABLE_CSS, Query::ARRAY_A);
	...
	public function get_all_image_sizes($type = 'gallery'){
		...
		switch($type){
			...
			case 'gallery':
			default:
				$added_image_sizes = $this->_frameworkHelper->get_intermediate_image_sizes();
