...
class RevSliderNavigation extends RevSliderFunctions {
	...
	public function init_by_id($nav_id){
		...
		$row = $wpdb->get_row($wpdb->prepare("SELECT `id`, `handle`, `type`, `css`, `settings` FROM ".$wpdb->prefix.RevSliderFront::TABLE_NAVIGATIONS." WHERE `id` = %d", $nav_id), Query::ARRAY_A);
	...
	public function get_all_navigations_short(){
		...
		$navigations = $wpdb->get_results("SELECT `id`, `handle`, `name` FROM ".$wpdb->prefix.RevSliderFront::TABLE_NAVIGATIONS, Query::ARRAY_A);
	...
	public function get_all_navigations($defaults = true, $raw = false, $old = false){
		...
		$navigations = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix.RevSliderFront::TABLE_NAVIGATIONS, Query::ARRAY_A);
		...
		if($defaults){
			...
			$default_presets = $this->_frameworkHelper->get_option('revslider-nav-preset-default', array());
	...
	public static function get_default_navigations(){
		...
		$navigations = Navigations::getNavigations($navigations);

		return self::$frameworkHelper->apply_filters('revslider_mod_default_navigations', $navigations);
	}
		...
		foreach($navs as $nav){
			if($nav['id'] == $data['navigation']){ //found the navigation, get ID and update settings
				...
				if(isset($nav['factory']) && $nav['factory'] == true){
						...
						foreach($default_presets[$nav['id']] as $prkey => $preset){
							if($prkey == $data['handle']){
								...
								$default_presets[$nav['id']][$prkey] = array(
									'name'		=> $this->_frameworkHelper->esc_attr($data['name']),
									//'handle'	=> $this->_frameworkHelper->esc_attr($data['handle']),
									'type'		=> $this->_frameworkHelper->esc_attr($data['type']),
									'values'	=> $data['values'],
									'editable'	=> true
								);
					...
					$this->_frameworkHelper->update_option('revslider-nav-preset-default', $default_presets);
