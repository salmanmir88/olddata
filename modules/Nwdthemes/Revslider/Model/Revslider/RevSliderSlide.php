...
class RevSliderSlide extends RevSliderFunctions {
	...
	public function get_settings(){
		return $this->_frameworkHelper->apply_filters('revslider_slide_get_settings', $this->_frameworkHelper->apply_filters('revslider_slide_getSettings', $this->settings, $this), $this);
	}
	...
	public function get_params(){
		return $this->_frameworkHelper->apply_filters('revslider_slide_get_params', $this->_frameworkHelper->apply_filters('revslider_slide_getParams', $this->params, $this), $this);
	}
	...
	public function get_layers(){
		return $this->_frameworkHelper->apply_filters('revslider_get_layers', $this->_frameworkHelper->apply_filters('revslider_getLayers', $this->layers, $this), $this);
	}
	...
	public function get_static_slide_id($slider_id){
		...
		$slide = array();
		if(!empty($slider_id)){
			$slide = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES." WHERE slider_id = %d", $slider_id), Query::ARRAY_A);
		}
	...
	public function exist_by_id($slideid){
		...
		try{
			if(strpos($slideid, 'static_') !== false){
				...
				$record = $wpdb->get_row($wpdb->prepare("SELECT id FROM ". $wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES ." WHERE id = %d", $slide_id), Query::ARRAY_A);
			}else{
				$record = $wpdb->get_row($wpdb->prepare("SELECT id FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE id = %d", $slideid), Query::ARRAY_A);
			}
		}catch(\Exception $e){
			return false;
		}
	...
	public function init_by_id($slide_id){
		...
		try{
			if(strpos($slide_id, 'static_') !== false){
				...
				$slide = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES ." WHERE id = %d", $static_id), Query::ARRAY_A);
			}else{
				$this->validate_numeric($slide_id, 'Slide ID');
				$slide = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE id = %d", $slide_id), Query::ARRAY_A);
			}
	...
	public function init_by_slide(RevSliderSlide $slide){
		$slide = $this->_frameworkHelper->apply_filters('revslider_slide_initBySlide', $slide, $this);
	...
	public function save_slide($slide_id, $data, $slider_id){
		...
		$this->settings['version'] = $this->get_val($params, 'version', $this->get_val($this->settings, 'version', Framework::RS_REVISION));
		...
		$this->_frameworkHelper->do_action('revslider_slide_updateSlideFromData_post', false, $data, $this);
	...
	public function save_slide_advanced($slide_id, $data, $slider_id){
		...
		$params = $this->get_val($data, 'params', array());
		$params = $this->json_decode_slashes($params);
		$version = $this->get_val($params, 'version', $this->get_val($this->settings, 'version', Framework::RS_REVISION));
		if(!empty($params)){
			if(isset($params['version'])) unset($params['version']);
	...
	public function delete_slide_by_id($slide_id){
		...
		$this->_frameworkHelper->do_action('revslider_slide_deleteSlide', $slide_id);
	...
	public function duplicate_slide_by_id($slide_id, $slider_id){
		...
		$done	= false;
		$slide	= $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE id = %s", $slide_id), Query::ARRAY_A);
		if(!empty($slide)){
	...
	public function init_by_post_data($data, RevSliderSlide $template, $slider_id){
		...
		$template_id	 = $this->_frameworkHelper->get_post_meta($post_id, 'slide_template', true);
		$template_id	 = ($template_id == '') ? 'default' : $template_id;
		$this->post_data = $this->_frameworkHelper->apply_filters('revslider_slide_initByPostData', $data, $template, $slider_id, $this);
		...
		if(!empty($template_id) && is_numeric($template_id)){ //init by local template, if this fails, init by global (slider) template
			try{
				...
				*/
				$local = new RevSliderSlide($this->_frameworkHelper);
		...
		if($this->get_val($this->params, array('seo', 'set'), false) == true && $this->get_val($this->params, array('seo', 'type'), 'regular') == 'regular'){
			$link = $this->_frameworkHelper->get_permalink($post_id);
			...
			if(!empty($matches)){
				foreach($matches as $match){
					...
					$meta_val = $this->_frameworkHelper->get_post_meta($post_id, $meta, true);
		...
		if($this->get_val($this->params, array('bg', 'imageFromStream'), false) === true){ //if image is choosen, use featured image as background // && $this->get_val($this->params, array('bg', 'type')) == 'image'
			$tid = $this->_frameworkHelper->get_post_thumbnail_id($post_id);

			if(!empty($tid)){
				$this->set_image_by_image_id($tid);

				//set the thumbnail image
				$thumbnail_url = $this->_frameworkHelper->getImagesHelper()->wp_get_attachment_image_src($tid, 'thumbnail');
	...
	private function set_layers_by_post($post, $slider_id){
		...
		$post = $this->_frameworkHelper->apply_filters('revslider_slide_setLayersByPostData_pre', $post, $slider_id, $this);

		$slider_source = $this->get_slider_param($slider_id, 'source', array());
		$excerpt_limit	= (int) str_replace('char', '', $this->get_val($slider_source, array('post', 'excerptLimit'), 55));
		$img_sizes	= $this->get_all_image_sizes();
		$ptid		= $this->_frameworkHelper->get_post_thumbnail_id($post['ID']);
		$attr		= array(
			'name'				=> $this->get_val($post, 'name'),
			'price'				=> $this->get_val($post, 'price'),
			'special_price'		=> $this->get_val($post, 'special_price'),
			'short_description'	=> $this->get_val($post, 'short_description'),
			'description'		=> $this->get_text_intro($this->get_val($post, 'description'), $excerpt_limit),
			'view_link'			=> $this->get_val($post, 'view_link'),
			'cart_link'			=> $this->get_val($post, 'cart_link'),
			'wishlist_link'		=> $this->get_val($post, 'wishlist_link'),
			'img_urls'			=> array()
		);

		foreach($img_sizes as $img_handle => $img_name){
			$featured_image_url = $this->_frameworkHelper->getImagesHelper()->wp_get_attachment_image_src($ptid, $img_handle);
		...
		$attr = $this->_frameworkHelper->apply_filters('revslider_slide_setLayersByPostData_post', $attr, $post, $slider_id, $this);

		if(!empty($this->layers)){
			foreach($this->layers as $key => $layer){
				$text = $this->get_val($layer, 'text');
				$text = $this->_frameworkHelper->apply_filters('revslider_mod_meta', $text, $post['ID'], $post); //option to add your own filter here to modify meta to your likings
				...
				if($this->get_val($layer, 'type', 'text') === 'image' && $this->get_val($layer, array('media', 'imageFromStream'), false) === true){
					$featured_image_url = $this->_frameworkHelper->getImagesHelper()->wp_get_attachment_image_src($ptid, 'full');
	...
	public function get_excerpt_by_id($id, $limit = 55){
		$post	 = $this->_frameworkHelper->get_post($id);
		...
		return $this->_frameworkHelper->apply_filters('revslider_getExcerptById', $excerpt, $post, $limit);
	}
	...
	public function set_post_data($text, $attr, $post_id){
		...
		$text = $this->_frameworkHelper->apply_filters('rev_slider_insert_meta', $text, $post_id);
		...
		$text = str_replace(array('%name%', '{{name}}'), $this->get_val($attr, 'name'), $text);
		$text = str_replace(array('%price%', '{{price}}'), $this->get_val($attr, 'price'), $text);
		$text = str_replace(array('%special_price%', '{{special_price}}'), $this->get_val($attr, 'special_price'), $text);
		$text = str_replace(array('%short_description%', '{{short_description}}'), $this->get_val($attr, 'short_description'), $text);
		$text = str_replace(array('%description%', '{{description}}'), $this->get_val($attr, 'description'), $text);
		$text = str_replace(array('%view_link%', '{{view_link}}'), $this->get_val($attr, 'view_link'), $text);
		$text = str_replace(array('%cart_link%', '{{cart_link}}'), $this->get_val($attr, 'cart_link'), $text);
		$text = str_replace(array('%wishlist_link%', '{{wishlist_link}}'), $this->get_val($attr, 'wishlist_link'), $text);
		...
		if(!empty($img_sizes)){
			foreach($img_sizes as $img_handle => $img_name){
				...
				$text = str_replace(array('%product_image_url_'.$img_handle.'%', '{{product_image_url_'.$img_handle.'}}'),  $this->get_val($attr, array('img_urls', $img_handle, 'url'), ''), $text);
				$text = str_replace(array('%product_image_'.$img_handle.'%', '{{product_image_'.$img_handle.'}}'), $this->get_val($attr, array('img_urls', $img_handle, 'tag'), ''), $text);
		...
		if(!empty($arrMatches)){
			foreach($arrMatches as $matched){
				foreach($matched as $match){
					...
					$meta = str_replace('%', '',$meta);
					$meta = str_replace('_REVSLIDER_', '-', $meta);
					$metaValue = $this->_frameworkHelper->get_post_meta($post_id, $meta, true);
		...
		if(!empty($arrMatches)){
			foreach($arrMatches as $matched){
				foreach($matched as $match) {
					...
					$avatar = $this->_frameworkHelper->get_avatar_url($this->get_val($attr, 'authorID'), array('size'=> $vals[0]));
	...
	public function init_by_stream_data($data, $template, $slider_id, $sourcetype, $additions){
		$a = $this->_frameworkHelper->apply_filters('revslider_slide_initByStreamData', array('post_data' => $data, 'template' => $template, 'slider_id' => $slider_id, 'sourcetype' => $sourcetype, 'additions' => $additions), $this);
		...
		switch($a['sourcetype']){
			...
			break;
			default:
				$return = $this->_frameworkHelper->apply_filters('revslider_slide_initByStreamData_sourceType', false, $a, $this);
	...
	private function init_by_facebook($slider_id, $additions){
		$this->post_data = $this->_frameworkHelper->apply_filters('revslider_slide_initByFacebook_pre', $this->post_data, $slider_id, $additions, $this);
		...
		if($this->get_val($this->params, array('bg', 'type')) == 'image'){ //if image is choosen, use featured image as background
			...
			$this->image_url = (empty($this->image_url)) ? Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/facebook.png' : $this->image_url;
			$this->image_url = ($this->_frameworkHelper->is_ssl()) ? str_replace('http://', 'https://', $this->image_url) : $this->image_url;
			$this->image_filename = basename($this->image_url);
		}

		$this->post_data = $this->_frameworkHelper->apply_filters('revslider_slide_initByFacebook_post', $this->post_data, $slider_id, $additions, $this);
	...
	private function init_by_twitter($slider_id, $additions){
		$this->post_data = $this->_frameworkHelper->apply_filters('revslider_slide_initByTwitter_pre', $this->post_data, $slider_id, $additions, $this);
		...
		if($bg_type == 'trans' || $bg_type == 'image' || $bg_type == 'streamtwitter' || $bg_type == 'streamtwitterboth'){ //if image is choosen, use featured image as background
			...
			if($img == ''){
				...
				if(is_array($image_url_array) && isset($image_url_array[0])){
					$img = ($this->_frameworkHelper->is_ssl()) ? $this->get_val($image_url_array[0], 'media_url_https') : $this->get_val($image_url_array[0], 'media_url');
				}
			}
			...
			$this->image_url = (empty($this->image_url)) ? Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/twitter.png' : $this->image_url;
			$this->image_url = ($this->_frameworkHelper->is_ssl()) ? str_replace("http://", "https://", $this->image_url) : $this->image_url;
			$this->image_filename = basename($this->image_url);
		}

		$this->post_data = $this->_frameworkHelper->apply_filters('revslider_slide_initByTwitter_post', $this->post_data, $slider_id, $additions, $this);
	...
	private function init_by_instagram($slider_id, $additions = array()){
		$this->post_data = $this->_frameworkHelper->apply_filters('revslider_slide_initByInstagram_pre', $this->post_data, $slider_id, $this);
		...
		if(in_array($this->get_val($this->params, array('bg', 'type')), array('trans', 'image', 'streaminstagram', 'streaminstagramboth'), true)){ //if image is choosen, use featured image as background
			...
			*/
			...
			$this->image_url = (empty($this->image_url)) ? Framework::$RS_PLUGIN_URL . 'public/assets/assets/sources/instagram.png' : $this->image_url;
		...
		$this->post_data = $this->_frameworkHelper->apply_filters('revslider_slide_initByInstagram_post', $this->post_data, $slider_id, $this);
	...
	private function init_by_flickr($slider_id, $additions){
		$this->post_data = $this->_frameworkHelper->apply_filters('revslider_slide_initByFlickr_pre', $this->post_data, $slider_id, $this);
		$this->id		 = $this->get_val($this->post_data, 'id');
		$this->set_param('title', $this->get_val($this->post_data, 'title'));
		$this->set_param(array('publish', 'state'), 'published');
		...
		if($this->get_val($this->params, array('bg', 'type')) == 'image'){ //if image is choosen, use featured image as background
			...
			$this->image_url	= (empty($this->image_url)) ? Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/flickr.png' : $this->image_url;
			$this->image_url	= ($this->_frameworkHelper->is_ssl()) ? str_replace("http://", "https://", $this->image_url) : $this->image_url;
			$this->image_filename = basename($this->image_url);
		}

		$this->post_data = $this->_frameworkHelper->apply_filters('revslider_slide_initByFlickr_post', $this->post_data, $slider_id, $this);
	...
	private function init_by_youtube($slider_id, $additions){
		$this->post_data = $this->_frameworkHelper->apply_filters('revslider_slide_initByYoutube_pre', $this->post_data, $slider_id, $additions, $this);
		...
		if(in_array($bg_type, array('trans', 'image', 'streamyoutube', 'streamyoutubeboth', 'youtube', 'streamvimeo', 'streamvimeoboth', 'vimeo'), true)){ //if image is choosen, use featured image as background
			...
			$this->image_url = (empty($this->image_url)) ? Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/youtube.png' : $this->image_url;
			$this->image_url = ($this->_frameworkHelper->is_ssl()) ? str_replace('http://', 'https://', $this->image_url) : $this->image_url;
		...
		$this->post_data = $this->_frameworkHelper->apply_filters('revslider_slide_initByYoutube_post', $this->post_data, $slider_id, $additions, $this);
	...
	private function init_by_vimeo($slider_id, $additions){
		$this->post_data = $this->_frameworkHelper->apply_filters('revslider_slide_initByVimeo_pre', $this->post_data, $slider_id, $additions, $this);
		...
		if($this->get_val($this->params, array('bg', 'imageFromStream'), false) === true && in_array($this->get_val($this->params, array('bg', 'type')), array('trans', 'image', 'streamvimeo', 'streamvimeoboth', 'vimeo'), true)){ //if image is choosen, use featured image as background
			...
			$this->image_url = $this->get_val($is, $img_res, '');
			$this->image_url = (empty($this->image_url)) ? Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/vimeo.png' : $this->image_url;
			$this->image_url = ($this->_frameworkHelper->is_ssl()) ? str_replace("http://", "https://", $this->image_url) : $this->image_url;
			$this->image_thumb = $this->get_val($is, 'thumbnail', $this->image_thumb);
			$this->image_filename = basename($this->image_url);
		}

		$this->post_data = $this->_frameworkHelper->apply_filters('revslider_slide_initByVimeo_post', $this->post_data, $slider_id, $additions, $this);
	...
	public function set_stream_data($text, $attr, $stream_type, $additions = array(), $is_action = false){
		...
		$text = $this->_frameworkHelper->apply_filters('revslider_slide_set_stream_data_pre', $text, $attr, $stream_type, $additions, $is_action, $img_sizes);
		...
		return $this->_frameworkHelper->apply_filters('revslider_slide_set_stream_data_post', $text, $attr, $stream_type, $additions, $is_action, $img_sizes);
	}
	...
	private function set_layers_by_stream($slider_id, $stream_type, $additions = array()){
		$a				= $this->_frameworkHelper->apply_filters('revslider_slide_setLayersByStreamData_pre', array('layers' => $this->layers, 'params' => $this->params), $slider_id, $stream_type, $additions, $this);
		...
		if(!empty($this->layers)){
			foreach($this->layers as $key => $layer){
				$text = $this->get_val($layer, 'text');
				$text = $this->_frameworkHelper->apply_filters('revslider_mod_stream_meta', $text, $slider_id, $stream_type, $this->post_data); //option to add your own filter here to modify meta to your likings
		...
		$a = $this->_frameworkHelper->apply_filters('revslider_slide_setLayersByStreamData_post', array('layers' => $this->layers, 'params' => $this->params), $slider_id, $stream_type, $additions, $this);
	...
	public function return_stream_data($stream_type, $additions = array()){
		$img_sizes = $this->get_all_image_sizes($stream_type);
		$attr = array();
		$attr = $this->_frameworkHelper->apply_filters('revslider_slide_return_stream_data_pre', $attr, $stream_type, $additions, $img_sizes);
		...
		switch($stream_type){
			...
			case 'instagram':
				...
				$attr1 = array(
					...
					'date'		=> $this->_frameworkHelper->date_i18n($this->_frameworkHelper->get_option('date_format').' '.$this->_frameworkHelper->get_option('time_format'), $this->get_val($this->post_data, 'taken_at_timestamp', false)),
		...
		return $this->_frameworkHelper->apply_filters('revslider_slide_return_stream_data_post', $attr, $stream_type, $additions, $img_sizes);
	}
	...
	public function get_facebook_timeline_image(){
		...
		return $this->_frameworkHelper->apply_filters('revslider_slide_get_facebook_timeline_image', $return, $object_id, $picture, $this);
	}
	...
	public function save_layers(){
		...
		$this->layers	= $this->_frameworkHelper->apply_filters('revslider_slide_saveLayers', $this->layers, $this->static_slide, $this);
	...
	public function save_params(){
		...
		$table = ($this->static_slide) ? $wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES : $wpdb->prefix . RevSliderFront::TABLE_SLIDES;
		$this->params = $this->_frameworkHelper->apply_filters('revslider_slide_saveParams', $this->params, $this->static_slide, $this);
	...
	public function save_settings(){
		...
		$table = ($this->static_slide) ? $wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES : $wpdb->prefix . RevSliderFront::TABLE_SLIDES;
		$this->settings = $this->_frameworkHelper->apply_filters('revslider_slide_save_settings', $this->settings, $this->static_slide, $this);
	...
	public function get_children(){
		...
		return $this->_frameworkHelper->apply_filters('revslider_slide_getArrChildren', $this->children, $this);
	}
	...
	public function create_slide($slider_id, $obj = '', $static = false, $id = false){
		...
		$insert	= $this->_frameworkHelper->apply_filters('revslider_slide_createSlide', $insert, $slider_id, $static, $this);
	...
	public function init_by_static_id($slide_id){
		...
		$slide = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES." WHERE id = %d", $slide_id), Query::ARRAY_A);
	...
	public function init_by_data($slide){
		$slide = $this->_frameworkHelper->apply_filters('revslider_slide_init_by_data', $this->_frameworkHelper->apply_filters('revslider_slide_initByData', $slide));
		...
		$image_url			= ($this->_frameworkHelper->is_ssl()) ? str_replace('http://', 'https://', $image_url) : $image_url;
	...
	public function get_slides_for_library($slider_ids = array(), $get_static_slide = false){
		...
		if(!empty($slider_ids)){
			foreach($slider_ids as $sid){
				$cur_slides = $wpdb->get_results($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE slider_id = %s", $sid), Query::ARRAY_A);

				if($get_static_slide === true){
					$static_slide = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES ." WHERE slider_id = %s", $sid), Query::ARRAY_A);
		...
		if(!empty($slides)){
			...
			foreach($slides as $key => $slide){
				...
				if($this->get_val($slide, 'static', false) === true){
					...
					$slides[$key]['bg'] = array('type' => 'image', 'src' => Framework::$RS_PLUGIN_URL.'admin/assets/images/sources/static.png', 'style' => array());
				}else{
					if($this->get_val($params, 'title', false) === false) $params['title'] = 'Slide '.$i;
					$slides[$key]['title']	= $this->get_val($params, 'title');

					$rslide = new RevSliderSlide($this->_frameworkHelper);
	...
	public function get_slides_by_slider_id($slider_id, $published = false, $wmpl = false, $first = false, $init_layer = true){
		...
		$slides		 = array();
		$children	 = array();
		$slides_data = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$wpdb->prefix . RevSliderFront::TABLE_SLIDES." WHERE slider_id = %d ORDER BY slide_order ASC", $slider_id), Query::ARRAY_A);

		foreach($slides_data as $slide_data){
			$slide	= new RevSliderSlide($this->_frameworkHelper);
			...
			*/
			...
			if ($published) {
				$storeId = $slide->get_param(array('store_id'), false);
				if ($storeId) {
					$storeIds = explode(',', $storeId);
					$storePermissions = ! in_array(0, $storeIds);
				} else {
					$storePermissions = $slide->get_param(array('store_permissions'), false);
					$storeIds = [];
					foreach ($slide->get_param(array('allow_stores'), []) as $key => $value) {
						if ($value) {
							$storeIds[] = str_replace('store', '', $key);
						}
					}
				}
				if ($storePermissions && ! in_array($this->_frameworkHelper->getStoreId(), $storeIds)) {
					continue;
				}
			}
	...
	public function get_params_for_export(){
		...
		return $this->_frameworkHelper->apply_filters('revslider_slide_getParamsForExport', $this->_frameworkHelper->apply_filters('revslider_slide_getParamsForExport', $params, $this), $this);
	}
	...
	public function get_layers_for_export(){
		...
		return $this->_frameworkHelper->apply_filters('revslider_slide_get_layers_for_export', $this->_frameworkHelper->apply_filters('revslider_slide_getLayersForExport', $layers, $this), $this);
	}
		...
		if($thumb_src === false){
			if($bgtype == 'trans'){
				...
				if(!empty($bg)){
					...
					$_src = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/';
					switch($layouttype){
						case 'gallery':
						break;
						case 'posts':
	...
	public function get_image_attributes($layouttype){
		...
		return $this->_frameworkHelper->apply_filters('revslider_slide_get_image_attributes', array(
			'type'	=> $class,
			'src'	=> $thumb,
			'style'	=> $style
		), $this);
	...
	public function get_used_fonts($full = false){
		...
		return $this->_frameworkHelper->apply_filters('revslider_slide_getUsedFonts', $fonts, $this);
	}
	...
	private function set_image_by_image_id($id){
		$id			= $this->_frameworkHelper->apply_filters('revslider_slide_setImageByImageID', $id, $this);
	...
	public function set_image_by_id($imageID, $size = 'full'){
		$a = $this->_frameworkHelper->apply_filters('revslider_slide_setImageByID', array('imageID' => $imageID, 'size' => $size), $this);
	...
	public function get_categories_html($cat_ids, $tax = null, $post_id = ''){
		...
		if(!empty($errors)){
			...
			$this->throw_error(__('retrieving categories error: '.$this->_frameworkHelper->esc_html($err)));
		}
		...
		foreach($categories as $category){
			...
			$link = $this->_frameworkHelper->get_category_link($category['term_id']);
			$name = $category['name'];
			$list.= ($sep == true) ? ',' : '';
			$list.= (!empty($link)) ? '<a href="' . $this->_frameworkHelper->esc_url($link) . '" title="' . $this->_frameworkHelper->esc_attr(sprintf(__('View all posts in %s', 'revslider'), $category['name'])) .'" '. $rel .'>'. $name .'</a>' : $name;
			$sep  = true;
		}
