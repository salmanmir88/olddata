...
class Framework extends \Magento\Framework\App\Helper\AbstractHelper {
	...
	const RS_REVISION = '6.2.15.5';
	const RS_TP_TOOLS = '6.2.15';
	...
	public static $rs_double_jquery_script = false;
	...
	public static $revslider_is_preview_mode = false;
	public static $revslider_save_post = false;
	...
	public function is_admin() {
		return $this->_noAdmin ? false : $this->_areaCode == 'adminhtml';
	}
	...
	public function get_transient($handle) {
		$data = $this->_cacheProxy->load('nwdthemes-revslider-transient-' . $handle);
		if ($data !== false) {
	...
	public function set_transient($handle, $value, $expiration = 0) {
		$this->_cacheProxy->save(serialize($value), 'nwdthemes-revslider-transient-' . $handle, array(), $expiration);
	}
	...
	public function delete_transient($handle) {
		$this->_cacheProxy->remove('nwdthemes-revslider-transient-' . $handle);
	}
	...
	public function requireScript($handle, $script) {
        ...
        $this->_registerHelper->setToRegister('require_scripts', $handle, $data);
	}
	...
	public function includeRequiredScripts() {
		$html = '';
		$scripts = $this->_registerHelper->getFromRegister('require_scripts');
		if ($scripts) {
			$_scripts = [''];
	...
	public function generatePreview($data) {
		...
		if ( ! $blankPage = $this->get_option('preview_blank_page')) {
			$this->preloadPreviewPage();
			$blankPage = $this->get_option('preview_blank_page');
		}

		$headIncludes = $this->_layoutFactory->create()
			->createBlock('Nwdthemes\Revslider\Block\Head')
			->setTemplate('Nwdthemes_Revslider::head.phtml')
			->assign($data)
			->toHtml();

		$data['headIncludes'] = $headIncludes;
		$data['frameworkHelper'] = $this;
		...
		$result = base64_encode(str_replace('</body>', $content . '</body>', $blankPage));

		return $result;
	}

	/**
	 * Preload public page for preview
	 */
	public function preloadPreviewPage() {
		$request = $this->wp_remote_get($this->_storeManager->getStore()->getBaseUrl() . 'nwdthemes_revslider/index');
		if ($this->wp_remote_retrieve_response_code($request) == 200) {
			$this->update_option('preview_blank_page', $this->wp_remote_retrieve_body($request));
		}
	}
	...
	public function sliderSaveDataFilter($data) {
		$data['params'] = json_encode($this->_makeImagesRelative(json_decode($data['params'], true)));
		return $data;
	}
	...
	public function sliderInitByDataFilter($data) {
		if ( ! empty($data['params'])) {
			$data['params'] = json_encode($this->_makeImagesAbsolute(json_decode($data['params'], true)));
		}
		return $data;
	}
	...
	public function slideInitByDataFilter($data) {
		...
		if ( ! empty($data['layers'])) {
			$data['layers'] = json_encode($this->_makeImagesAbsolute(json_decode($data['layers'], true)));
		}
		return $data;
	}
	...
	protected function _makeImagesRelative($data) {
		...
		} elseif (is_string($data) && $this->getImagesHelper()->isMedia($data) && $data !== $this->getImagesHelper()->imageFile($data)) {
	...
	protected function _makeImagesAbsolute($data) {
		...
		} elseif (is_string($data) && strpos($data, Images::MEDIA_URL_PACEHOLDER) === 0) {
