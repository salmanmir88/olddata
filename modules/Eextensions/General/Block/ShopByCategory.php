...
class ShopByCategory extends \Magento\Framework\View\Element\Template
{
    ...
    {		
		...
		$collection->addFieldToFilter('show_on_brandpage', 1);
        $collection->addAttributeToSort('position', 'ASC');

        $catData = array();
        foreach ($collection as $key=>$categoryData) {
			if($categoryData->getShowOnBrandpage() == 1){
				...
				$catData[$key]['show_on_brandpage'] 	= $categoryData->getShowOnBrandpage();
			}
		}
        return $catData;
    }
    ...
    */
    ...
    public function buildTree(array $elements, $parentId = 36) {
		$branch = array();
